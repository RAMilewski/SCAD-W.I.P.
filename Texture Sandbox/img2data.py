...
def img2tex(filename, varname, invert, hflip, resize, outf):
    indent = " " * 4
...
def main():
    parser = argparse.ArgumentParser(prog='img2data')
    ...
    parser.add_argument ('-f', '--hflip', action='store_true',
    ...
    if opts.outfile:
        with open(opts.outfile, "w") as outf:
            img2tex(opts.infile, opts.varname, opts.invert, opts.hflip, opts.resize, outf)
    else:
        img2tex(opts.infile, opts.varname, opts.invert, opts.resize, opts.hflip, sys.stdout)
