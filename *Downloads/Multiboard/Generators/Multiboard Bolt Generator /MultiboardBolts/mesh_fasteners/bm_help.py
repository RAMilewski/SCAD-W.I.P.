...
from .bm_filters import dict_by_type
...
def set_attib_values(elements, attrib, value):
    for elem in elements:
        elem[attrib] = value


def all_geom(bm: bmesh.types.BMesh):
    """
    returns all geometry from bm
    """
...
def verts_to_circle(verts: List[BMVert], radius: float):
...
def islands(bm: BMesh, verts: Iterable[BMVert]) -> List[List[BMVert]]:
    """ Sort verts into lists based on whether they can be found through connecting edges """
...
def merge_bmesh(bm_a: BMesh, bm_b: BMesh) -> Tuple[List[BMVert], List[BMFace]]:
    ...
    WARNING: Merges onlf verts and faces, no atributes.
    """
...
# def limited_dissolve(bm: BMesh, angle: float = radians(1.7)):
