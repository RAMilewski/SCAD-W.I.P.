...
part = "handle";    // roller, handle, axle, test
...
h_round = 4;
h_fudge = 2; 
handle = [100, h_size, h_size/2];
forkA = [h_size, h + h_size/2 + h_fudge, h_size/2];
forkB = [h_size + r + r_rod +2, h_size, h_size/2];
...
module roller() {   
    diff() {
        ...
        cyl(h, r, texture = texture,  tex_reps = tex_reps, tex_depth = tex_depth, anchor = BOT) {
            ...
            attach(TOP) cyl(depth * .66, r1 = r, r2 = r + depth, anchor = TOP);
        }
