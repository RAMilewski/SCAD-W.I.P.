...
function nurbs_curve(control,degree,splinesteps,u,  mult,weights,type="clamped",knots,deriv=0) =
  ...
  : assert(is_undef(splinesteps) || (is_int(splinesteps) && splinesteps>0), "splinesteps must be a positive integer")
    ...
    is_def(weights) ? assert(is_vector(weights, len(control)), "Weights should be a vector whose length is the number of control points")
                      assert(!is_list(deriv), "Weighted NURBS derivatives are not yet implemented")
                      let(
                           dim = len(control[0]),
...
// Recursively build d-th derivative local control points.
// local_ctrl: p+1-d_so_far elements for the current knot span
// p: original degree; d_so_far: steps already taken; target_d: desired order
// Formula: Q^(d)[i] = (p-d+1)*(Q^(d-1)[i+1]-Q^(d-1)[i]) / (knot[k+i+1]-knot[k-p+i+d])
...
// Evaluate d-th derivative of a B-spline at parameter u in knot span k.
// local_ctrl: p+1 local control points (global indices k-p through k)
