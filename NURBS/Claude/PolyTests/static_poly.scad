...
deriv = [undef, undef, undef, undef, undef, undef, undef, undef];

curve = [undef, undef, undef, undef, undef, undef, undef, undef];
...
debug_nurbs_interp(data,3, deriv = deriv, curvature = curve, splinesteps=32, type = "closed", show_ctrl=false, color=color2, data_color="red", data_size=1, method=method2);
if (showpoly) stroke(data, closed = true, width = 0.5);
echo(data);
