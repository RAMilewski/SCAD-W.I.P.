...
//                 are unconstrained.  Clamped type only.  Cannot be combined
//                 with start_der=/end_der=.  Default: undef (all unconstrained)
...
function nurbs_interp(points, degree, centripetal=false, type="clamped",
    ...
    assert(is_undef(derivs) || type == "clamped",
           "nurbs_interp: derivs= only supported for type=\"clamped\"")
