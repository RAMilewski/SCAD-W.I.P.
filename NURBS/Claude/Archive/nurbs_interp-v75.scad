...
// Returns unit-length tangent vectors; preserves corners when
// three consecutive chords are collinear on one side (denom→0 ⇒ α=1).
function _five_point_tangents(points) =
    let(
        n = len(points) - 1
    )
    n <= 0 ? [points[0] * 0]      // degenerate: single point
  : let(
        ...
        // q_ext[0]=q[−1], q_ext[1]=q[0], q_ext[2..n+1]=q[1..n],
        // q_ext[n+2]=q[n+1], q_ext[n+3]=q[n+2]
        q_ext = [qm1, q0, each q, qn1, qn2]
    )
    [for (k = [0:1:n])
        let(
            // Book indices: q[k−1]=q_ext[k], q[k]=q_ext[k+1],
            //               q[k+1]=q_ext[k+2], q[k+2]=q_ext[k+3]
            qkm1 = q_ext[k],
            qk   = q_ext[k+1],
            qk1  = q_ext[k+2],
            qk2  = q_ext[k+3],
            ...
            alpha   = denom < 1e-15 ? 1 : cross_l / denom,
            // Eq 9.29
            V  = (1 - alpha) * qk + alpha * qk1,
            nV = norm(V)
        )
        nV < 1e-15 ? qk / max(norm(qk), 1e-15) : V / nV
    ];
