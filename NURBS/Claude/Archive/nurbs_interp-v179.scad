...
function nurbs_elevate_degree(control, degree, knots=undef,
  ...
  : times == 0
    ...
    let(
        ...
        // p_new endpoint copies back off for clamped so the output stays in xknots format.
        U_full = type == "clamped"
    ...
    : // Rational NURBS: convert to homogeneous, elevate, extract.
      let(
          ...
          r         = _elevate_once(homo_ctrl, degree, U_full),
          new_knots = type == "clamped"
                      ? [for (i = [degree+1:1:len(r[1])-degree-2]) r[1][i]]
                      : r[1],
      ...
      times == 1
      ? [type, r[2], new_ctrl, new_knots, undef, new_w]
      : nurbs_elevate_degree(new_ctrl, r[2], new_knots,
