...
// p          = target degree
// start_der  = derivative vector at start of first segment (undef if none)
// end_der    = derivative vector at end of last segment (undef if none)
...
//   u_edges        = list (or singleton) of interior row indices where C0
//                    creases run in the v-direction.  Creates sharp edges
//                    across the surface at the specified rows.  Requires
//                    type_u="clamped".  Compatible with flat_edges= and
//                    boundary derivatives in the u-direction.
//                    Default: undef
//   v_edges        = list (or singleton) of interior column indices where C0
//                    creases run in the u-direction.  Creates sharp edges
//                    across the surface at the specified columns.  Requires
//                    type_v="clamped".  Compatible with flat_edges= and
//                    boundary derivatives in the v-direction.
//                    Default: undef
//
// Returns:
        ...
        T_u_start = has_sud_eff
                  ? has_ve
                    ? _solve_with_edges(v_edge_sys, _su_der_data,
                                        v_params, ve_norm, p_v,
                          start_der = has_svd_eff ? zero_v : undef,
                          end_der   = has_evd_eff ? zero_v : undef)[0]
                    : linear_solve(N_v,
                          concat(_su_der_data,
                              has_svd_eff ? [zero_v] : [],
                              has_evd_eff ? [zero_v] : []))
        ...
        P_T_raw = has_ue
            ? [for (j = [0:n_v_ctrl-1])
                _solve_with_edges(u_edge_sys, R_T[j],
                    ...
                    start_der = has_sud_eff ? T_u_start[j] : undef,
                    end_der   = has_eud_eff ? T_u_end[j]   : undef)]
            : undef,
        P_T  = has_ue
            ? [for (r = P_T_raw) r[0]]
            : [for (j = [0:n_v_ctrl-1])
