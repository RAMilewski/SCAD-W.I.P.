...
// via Schur complement decomposition.  Falls back to penalized normal
// equations if the Schur complement is singular (rank-deficient A).
//
...
//
// Primary path (exact interpolation):
//   R_pd = R + ε·I   →   Z = R_pd⁻¹·A^T   →   S = A·Z   →   Λ = S⁻¹·rhs   →   P = Z·Λ
//
// Fallback (approximate, μ-weighted):
//   (R + μ·A^T·A)·P = μ·A^T·rhs
//
...
function _kkt_solve(R, A, rhs, eps=1e-6) =
    ...
    let(
        // Schur complement S = A · Z  (N_rows × N_rows)
    ...
    : // S is singular (rank-deficient A) — fall back to penalized
      // normal equations: min P^T·R·P + μ·‖A·P − rhs‖².
      let(
          mu    = 1e8,
          ATA   = [for (i = [0:1:M-1])
                      [for (j = [0:1:M-1])
