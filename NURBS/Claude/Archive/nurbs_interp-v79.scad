...
//
// The W=1 construction is special: the derivative magnitude formulas
// (Eqs. 3.27–3.28) are independent of the angle χ_k between adjacent
...
// average of adjacent chord vectors).  When the discriminant in
// Eqs. 3.27–3.28 is negative (dot products < 0.8), Ball's option (b)
// is applied: replace t_k · t_{k+1} with (s_k · t_k)(s_k · t_{k+1}).
...
function _lockyer_params(points, closed) =
    let(
        ...
        // Derivative magnitudes a_k, b_k for each segment (Eqs. 3.27–3.28).
        ab = [for (k = [0:1:nseg-1])
            let(
                k1  = (k + 1) % np,
                st  = s_dir[k] * tang[k],       // s_k · t_k
                st1 = s_dir[k] * tang[k1],      // s_k · t_{k+1}
                tt  = tang[k] * tang[k1],        // t_k · t_{k+1}
                // Discriminant: 25(st)²(st1)² - 16(st)(st1)(tt)
                disc_raw = 25 * st * st * st1 * st1
                         - 16 * st * st1 * tt,
                // Ball option (b): if disc < 0 replace tt with st*st1
                use_fb  = disc_raw < 0,
                tt_eff  = use_fb ? st * st1 : tt,
                disc    = use_fb ? 25 * st * st * st1 * st1
                                 - 16 * st * st1 * tt_eff
                                 : disc_raw,
                sqrtd   = sqrt(max(disc, 0)),
                denom   = 5 * st * st1 + sqrtd,
                // Guard against degenerate segments (tangent perpendicular
                // to chord).  Fall back to a_k = b_k = c_k (chord-length).
                safe    = abs(st) > 1e-10 && abs(st1) > 1e-10
                          && abs(denom) > 1e-15,
                a = safe ? 8 * c[k] * st1 / denom : c[k],
                b = safe ? 8 * c[k] * st  / denom : c[k]
            )
            [a, b]
        ],
...
function _interp_params(points, method="dynamic", closed=false) =
    ...
    total_raw < 1e-10
      ...
      : assert(min(raw) > 1e-10,
        ...
        // Lockyer has its own recurrence; bypass the cumsum path.
        method == "lockyer" ? _lockyer_params(points, closed)
      : let(
...
//   ---
//   method = parameterization method: "length" (chord-length), "centripetal" (square-root exponent, Lee 1989), "dynamic" (per-chord dynamic exponent, Balta et al. 2020), "foley" (centripetal + deflection-angle correction, Foley & Neilson 1987), or "lockyer" (W=1 orthogonal construction, Lockyer 2007 §3.1.3 / Ball 2004; derivative magnitudes satisfy orthogonality between first and second parametric derivatives at Gauss points).  Default: "dynamic"
