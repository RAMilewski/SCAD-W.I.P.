...
//   `[type, degree, ctrl_grid, knots, undef, undef, rotation, uv]` that can be passed
//   directly to `{{nurbs_vnf()}}`.  The 8th element `uv` is `[u_params, v_params]`,
//   the parameterization vectors used in the solve, which you can reference if
//   you need to know the NURBS parameter value pair that corresponds to a grid
//   point of your input.  
...
//   and `col_wrap=true` smoothly joins the first and last columns.  Both false (the default) gives a flat
...
//   surface.  `u_edges=` specifies the indices of rows that will be edges or creases,
//   and `v_edges=` specifies the indices of columns that will be edges or creases. 
//   For a wrapped direction, the smallest specified index becomes the seam and that
//   direction is handled as clamped.  For a non-wrapped direction, indices
...
//   flat_edges = 4-element list `[first_row, last_row, first_col, last_col]` of derivative scales at the four coplanar boundary edges.  Each entry is a scalar or per-point list; `undef` leaves that edge free.  Shorthand: `flat_edges=s` → `[s,s,s,s]`.  Requires `row_wrap=false, col_wrap=false`. 
...
//   u_edges   = Row indidces (or index) of rows that are treated as edges or creases.  
//   v_edges   = Column indices (or index) of columns that are treated as edges or creases
...
//   caps         = (Module form only) Cap both open boundary edges (mixed wrap only).  Default: false
//   cap1         = (Module form only) Cap the first open boundary edge.  
//   cap2         = (Module form only) Cap the second open boundary edge. 
