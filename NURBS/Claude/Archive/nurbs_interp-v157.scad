...
//   .
//   The `type` parameter can be "clamped" to specify a "clamped" type NURBS that
//   starts at its first point and ends at its last one, or it can be "closed"
//   to specify a "closed" nurbs which starts where it ends.  Under some circumstances
//   when you request a "closed" nurbs the return will be of type "clamped".
//   Choosing "closed" is different than duplicating the last point because in
//   the former case the curve is smooth at the closing point, whereas duplicating
//   the last point with type "clamped" will create a corner.  
//   .
...
//   .
//   Under various situations closed curves will not start at `point[0]`.  The index of the data point
//   that starts the curve is returned in `last(result)`: the first
//   point of the curve will be `points[last(result]]`.  
//
// Arguments:
...
//   result = nurbs_interp_surface(points, degree, [method=], [type=], [normal1=], [normal2=], [flat_edges=], [flat_end1=], [flat_end2=],
...
//   directly to `{{nurbs_vnf()}}`.  When called as a module, renders the NURBS surface as geometry.  
//   .
//   The `type=` parameter specifies the type in the `u` and `v` directions
//   as either `"clamped"` (the default) or `"closed"`.  When the type is `"clamped"`
//   then the curve comes to an edge or a point at the ends of the range.
//   When a curve is `"clamped"` in both directions the typical result is a curve
//   with four edges.  A `"closed"` direction wraps around itself.  If a curve
//   is `"closed"` in both directions the result is a torus.
//   A curve which is `["clamped","closed"]` or `["closed","clamped"]` generally
//   is a tube shape with two polygonal edges in the u direction for the first case
//   or the v direction for the second case.  By itself this is not a valid manifold object
//   in OpenSCAD due to its two edges.  You can use this geometro to create a valid object
//   by specifying degenerate edges, where every point on each edge is the same point.  In this case
//   the shape will close into a ball.  
//   .
...
//   .
//   **Starting Point for "closed" curves**  When a closed curve is shifted to align a seam with the starting
//   point the location is returned in `last(result)` as `[u_rot, v_rot]`.
// Arguments:
//   points = Rectangular grid of 3D data points
//   degree = Scalar or 2-vector giving the degree of the NURBS in the two directions.
//   ---
//   method    = Parameterization method: `"length"`, `"centripetal"`, `"dynamic"`, `"foley"`, or `"fang"`. Default: `"centripetal"`
//   type = A single string or pair of strings giving the NURBS type in the two directions, where each entry is one of `"clamped"` or `"closed"`.  Default: `"clamped"`
