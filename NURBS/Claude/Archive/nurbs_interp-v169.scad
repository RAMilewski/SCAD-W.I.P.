...
//
// The knot vector for the elevated curve is built by incrementing each distinct
// interior knot value's multiplicity by 1.  The endpoint positions (bar_knots[0]
// and bar_knots[-1]) always appear exactly once in xknots (the "bar_knots" for
// BOSL2), so that nurbs_curve(type="closed") can extend them correctly via
// _extend_knot_vector.  Only interior positions get one extra copy.
//
// The critical bug this implementation avoids: when Greville abscissae are taken
// starting from index 0 in U_new, the first few sites fall below a_new and must
// be shifted by +T.  Those shifted values may coincide exactly with unshifted
// sites later in the list (e.g. grev[0]+T == grev[18]) → singular collocation.
// Fix: find the first index `start` such that grev[start] >= a_new, then take
// n_new consecutive sites from grev[start..start+n_new-1].  Sites > b_new are
// wrapped by -T.  _greville() returns n_new+p_new values (enough headroom for
// any start offset up to p_new).
//
// Input:  ctrl      = n_old control points (no closing repeat)
//         p         = current degree
//         bar_knots = BOSL2-format closed knot vector: endpoints appear once,
//                     interior positions appear curr_m[i] times.
...
function _elevate_once_closed(ctrl, p, bar_knots, curr_mult=undef) =
    let(
        ...
        // xknots: BOSL2 bar_knots format — first endpoint once, interior × mult,
        // last endpoint × curr_m[B-1] (period endpoint T is periodic-interior,
        // so its multiplicity must be tracked and incremented like any other knot).
        xknots_old = B <= 2
        ...
        // Evaluate original curve at each Greville site.
        // All grev[k] ∈ [a_new, b_new) ⊆ [U_old[0], U_old[-1]] — _nip valid.
        C_vals = [for (u = grev)
