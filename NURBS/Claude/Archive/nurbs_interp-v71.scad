...
// Estimate Bessel tangent vectors at each data point using Newton divided
// differences.  Interior (k=2..n-2): 5-point estimate via quartic interpolant
// (P&T §9.3.1, Eq 9.29).  Near-endpoints (k=1, k=n-1): 3-point estimate
// (Eq 9.28).  Endpoints (k=0, k=n): 4-point estimate via cubic interpolant
// (Eq 9.31).  Falls back to 3-point when fewer than 4 data points.
// Returns a list of tangent vectors (one per data point).

function _bessel_tangents(points, params) =
    let(
        n = len(points) - 1,
        // First divided differences: d_k = (Q_{k+1} - Q_k) / (t_{k+1} - t_k)
        d = [for (k = [0:1:n-1])
            (points[k+1] - points[k]) / max(params[k+1] - params[k], 1e-15)],
        // Parameter intervals
        h = [for (k = [0:1:n-1]) max(params[k+1] - params[k], 1e-15)]
    )
    n == 0 ? [d[0]]
  : n == 1 ? [d[0], d[0]]
  : n == 2 ?
    // 3-point everywhere (Eq 9.28 interior, Eq 9.30 endpoints)
    let(
        T1 = (h[1] * d[0] + h[0] * d[1]) / (h[0] + h[1]),
        T0 = 2 * d[0] - T1,
        T2 = 2 * d[1] - T1
    ) [T0, T1, T2]
  : let(
        ...
        D2 = [for (k = [0:1:n-2])
            (d[k+1] - d[k]) / max(h[k] + h[k+1], 1e-15)],
        // Third divided differences
        D3 = [for (k = [0:1:n-3])
            (D2[k+1] - D2[k]) / max(h[k] + h[k+1] + h[k+2], 1e-15)],
        // Fourth divided differences (5-point interior, needs n >= 4)
        D4 = n < 4 ? [] : [for (k = [0:1:n-4])
            (D3[k+1] - D3[k]) / max(h[k] + h[k+1] + h[k+2] + h[k+3], 1e-15)],
        // 4-point endpoint T_0 (Eq 9.31): cubic through {Q_0..Q_3}
        T0 = d[0] - D2[0] * h[0] + D3[0] * h[0] * (h[0] + h[1]),
        // 4-point endpoint T_n (Eq 9.31): cubic through {Q_{n-3}..Q_n}
        H3 = h[n-3] + h[n-2] + h[n-1],
        H2 = h[n-2] + h[n-1],
        H1 = h[n-1],
        Tn = d[n-3] + D2[n-3] * (H3 + H2)
           + D3[n-3] * (H2 * H1 + H3 * H1 + H3 * H2),
        // 3-point near-endpoints (Eq 9.28)
        T1   = (h[1] * d[0] + h[0] * d[1]) / (h[0] + h[1]),
        Tn_1 = (h[n-1] * d[n-2] + h[n-2] * d[n-1]) / (h[n-2] + h[n-1]),
        // 5-point interior tangents (Eq 9.29): quartic through {Q_{k-2}..Q_{k+2}}
        interior5 = [for (k = [2:1:n-2])
            let(
                a = h[k-2] + h[k-1],
                b = h[k-1]
            )
            d[k-2] + D2[k-2] * (a + b) + D3[k-2] * a * b
                   - D4[k-2] * a * b * h[k]]
    )
    [T0, T1, each interior5, Tn_1, Tn];
