    ...
    let(
        ...
        R = [for (k = [0:n_rows-1])
            let(rhs = concat(
                    points[k],
                    has_svd
                        ? [_force_deriv_dim(start_v_der[k], dim) * v_path_lens[k]]
                        : [],
                    has_evd
                        ? [_force_deriv_dim(end_v_der[k], dim) * v_path_lens[k]]
                        : []))
        ...
        T_u_start = has_sud
                  ? linear_solve(N_v,
                        concat(
                            [for (l = [0:n_cols-1])
                                _force_deriv_dim(start_u_der[l], dim) * u_path_lens[l]],
                            has_svd ? [zero_v] : [],
                            has_evd ? [zero_v] : []))
                  : undef,
        T_u_end   = has_eud
                  ? linear_solve(N_v,
                        concat(
                            [for (l = [0:n_cols-1])
                                _force_deriv_dim(end_u_der[l], dim) * u_path_lens[l]],
                            has_svd ? [zero_v] : [],
                            has_evd ? [zero_v] : []))
                  : undef,
