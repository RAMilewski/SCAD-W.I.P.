...
// Clamped interpolation with C0 corner joints via independent segments.
//
...
//
// Returns a segmented result [undef, undef, 0, segments] where each segment
// is [control, knots, seg_p].  Callers use _nurbs_is_segmented() to detect
// and must read seg[2] for the per-segment degree (not the global degree).
//
// corner_idxs — sorted list of interior split indices from eff_der.
// NaN entries are replaced with undef at the shared endpoint of each pair of
// adjacent segments so each sub-segment sees clean free endpoints.
