...
function _elevate_once_closed(ctrl, p, bar_knots, curr_mult=undef) =
    let(
        ...
        // Uniform collocation sites: τ_k = a_new + (k+0.5)*T/n_new, k=0..n_new-1.
        // Always distinct (spacing T/n_new > 0), never land on knot boundaries (the
        // +0.5 offset keeps them interior), and free of wrapping ambiguities — unlike
        // Greville abscissae which collide for high-multiplicity interior knots.
