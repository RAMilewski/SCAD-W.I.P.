...
function _nurbs_interp_closed(points, degree, method, deriv, curvature,
    ...
    : let(
        ratio     = _ctrl_point_ratio(points, raw[0]),
        threshold = pow(2, p) / p
      )
      ratio <= threshold ? raw
      : let(
...
function _closed_constrained_solve(points, p, method, eff_der, eff_curv, rot) =
    let(
        ...
        // params, then insert one knot per constraint at the midpoint
        // of the span containing its parameter (largest span first).
        // _fix_tiny_spans cleans up any near-coincident knots.
