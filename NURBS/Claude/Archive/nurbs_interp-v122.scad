...
function _nullspace_solve(R, A, rhs, eps=1e-6) =
    ...
    M == N_rows ? linear_solve(A, rhs)   // Square: unique solution.
    : let(
        // Minimize x^T·R·x subject to A·x = b via KKT saddle-point system.
        //
        //   [R+ε·I   A^T ] [x]   [0]
        //   [A       0   ] [λ] = [b]
        //
        // This (M+N)×(M+N) square system is solved by BOSL2 linear_solve.
        // The upper-left regularization (ε·I) ensures R+ε·I is SPD even
        // when R is only PSD (e.g. first-difference smoothing at boundaries).
        zero_d = is_list(rhs[0]) ? [for (d = [0:1:len(rhs[0])-1]) 0] : 0,
        R_pd   = [for (i = [0:1:M-1])
                      [for (j = [0:1:M-1]) R[i][j] + (i == j ? eps : 0)]],
        KKT    = concat(
            [for (i = [0:1:M-1])
                concat(R_pd[i], [for (j = [0:1:N_rows-1]) A[j][i]])],
            [for (i = [0:1:N_rows-1])
                concat(A[i],    [for (j = [0:1:N_rows-1]) 0])]
        ),
        rhs_kkt = concat([for (i = [0:1:M-1]) zero_d], rhs),
        sol     = linear_solve(KKT, rhs_kkt)
    )
    // Fall back to minimum-norm particular solution if KKT is degenerate.
    sol == [] ? linear_solve(A, rhs)
    : [for (i = [0:1:M-1]) sol[i]];
...
//   lying in the edge plane, oriented toward the ring (adjacent interior row/column).
//   scales sets magnitude per point (scalar or list); positive = inward (closes surface),
//   negative = outward (flares surface).  Same convention at both start and end edges —
//   no negation needed at end edges.
...
function _coplanar_inward_tangents(scales, edge, ring, periodic=false) =
    ...
    [for (j = [0:1:n-1])
        ...
        : let(
            ...
            // Orient toward ring (surface interior): project ring→edge vector
            // onto the edge plane, then choose the ±cand sign that aligns with it.
            R_raw = ring[j] - edge[j],
            R3    = dim == 2 ? [R_raw[0], R_raw[1], 0] : R_raw,
            R_in  = R3 - (R3 * P_hat) * P_hat,
            r_dot = cand * R_in,
            inward = r_dot >= 0 ? cand : -cand
        )
        sc[j] * inward
    ];
...
function nurbs_interp_surface(points, degree, method="centripetal", type="clamped",
    ...
    let(
        ...
        //   oriented toward the ring (surface interior) using the ring-direction reference.
        //   Positive scale closes inward, negative flares outward — same convention at
        //   both start (u=0) and end (u=1) boundaries; no negation needed at end edges.
