...
function nurbs_interp(points, degree, method="centripetal", type="clamped",
    ...
    [eff_type, degree, raw[0], raw[1], undef, undef];
...
// Synopsis: Visualizes interpolation: curve, control polygon, derivative arrows, curvature overlays.
...
//                      [curvature=], [start_curvature=], [end_curvature=], [corners=],
//                      [extra_pts=], [smooth=],
//                      [width=], [size=], [show_ctrl=], [show_knots=], [knots=],
//                      [color=], [data_color=], [data_size=]);
//
// Description:
//   Calls `nurbs_interp()` with the supplied arguments, then renders all of
//   the following overlays in a single call:
//   .
//   * **Curve** — stroked in `color` (or current color when `color=undef`).
//   * **Control polygon** — lightblue stroke + blue point markers, shown when `show_ctrl=true`.
//   * **Knot markers** — purple dots on the curve at each distinct interior knot
//     position, shown when `show_knots=true`.
//   * **Derivative arrows** — black arrows (half curve `width`, `"arrow2"` endcap)
//     drawn from each data point that carries a `deriv`, `start_deriv`, or
//     `end_deriv` constraint.  Arrow length scales with average inter-point
//     arc length.
//   * **Curvature osculating circles / disks** — `color([0,1,0,0.1])` (transparent green).
//     2-D: a circle of radius 1/κ centred on the osculating centre.
//     3-D: a thin disk (cylinder, `h=width/2`) lying in the osculating plane.
//     A zero-curvature constraint (κ = 0) is instead shown as a black line
//     segment through the data point along the local tangent direction.
//   * **Data points** — `data_color` (magenta by default) circle/sphere markers.
//
// Arguments:
...
//   deriv            = Per-point derivative constraints (list; undef entries = unconstrained).  Default: undef
//   start_deriv      = Derivative vector at first data point.  Default: undef
//   end_deriv        = Derivative vector at last data point.  Default: undef
//   curvature        = Per-point curvature-vector constraints (list; undef = unconstrained).  Default: undef
//   start_curvature  = Curvature vector at first data point.  Default: undef
//   end_curvature    = Curvature vector at last data point.  Default: undef
//   corners          = Corner-point indices for segmented curves.  Default: undef
...
//   size             = Control-point marker diameter override.  Default: 3*width
//   show_ctrl        = Render control polygon and control-point markers.  Default: true
//   show_knots       = Render knot-position markers on the curve.  Default: false
//   knots            = Knot vector passed to `debug_nurbs`; overrides `result[3]`.  Default: undef
//   color            = Curve color.  Default: undef (inherits current color)
//   data_color       = Data-point marker color.  Default: "magenta"
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="centripetal",
                          ...
                          width=1, size=undef, show_ctrl=true, show_knots=false,
                          knots=undef,
    ...
    ctrl      = result[2];
    knots_eff = is_undef(knots) ? result[3] : knots;
    curve     = nurbs_curve(result, splinesteps=splinesteps);
    clen      = path_length(curve);
    arrow_len = clen / max(np, 1) * 0.4;

    // Effective per-point derivative and curvature vectors (length np, undef = unconstrained)
    eff_deriv = [for (i = [0:1:np-1])
        i == 0      && !is_undef(start_deriv)                                       ? start_deriv
        : i == np-1 && !is_undef(end_deriv)                                         ? end_deriv
        : !is_undef(deriv) && i < len(deriv) && !is_undef(deriv[i])                 ? deriv[i]
        : undef
    ];
    eff_curv = [for (i = [0:1:np-1])
        i == 0      && !is_undef(start_curvature)                                   ? start_curvature
        : i == np-1 && !is_undef(end_curvature)                                     ? end_curvature
        : !is_undef(curvature) && i < len(curvature) && !is_undef(curvature[i])     ? curvature[i]
        : undef
    ];

    // --- Curve (via debug_nurbs with explicit kwargs — avoids BOSL2 contorl[4] typo) ---
    if (is_undef(color))
        debug_nurbs(ctrl, result[1],
                    ...
                    show_knots=false, show_control=false);
    else
        color(color)
            debug_nurbs(ctrl, result[1],
                        splinesteps=splinesteps, width=width, size=sz,
    ...
    // --- Control polygon (lightblue lines) and control-point markers (blue) ---
    if (show_ctrl) {
        color("lightblue")
            stroke(ctrl, width=width/2, closed=(result[0]=="closed"));
        color("blue")
            for (pt = ctrl)
                translate(pt)
                    if (is2d) circle(r=sz/2, $fn=16);
                    else      sphere(r=sz/2, $fn=16);
    }

    // --- Knot markers (purple dots on the curve) ---
    if (show_knots) {
        // Clamped: distinct interior knots from full knot vector.
        // Closed: all bar_knot positions except the last (periodic duplicate).
        knot_params = is_undef(result[4])
            ? [for (u = unique(knots_eff)) if (u > 1e-10 && u < 1 - 1e-10) u]
            : [for (k = [0:1:len(knots_eff)-2]) knots_eff[k]];
        for (u = knot_params)
            color("purple")
                translate(nurbs_curve(result, u=u))
                    if (is2d) circle(r=sz/3, $fn=8);
                    else      sphere(r=sz/3, $fn=8);
    }

    // --- Derivative arrows (black, half width, arrow2 endcap) ---
    for (i = [0:1:np-1])
        if (!is_undef(eff_deriv[i])) {
            dn = norm(eff_deriv[i]);
            if (dn > 1e-12)
                color("black")
                    stroke([points[i],
                            points[i] + eff_deriv[i] / dn * arrow_len],
                           width=width/2,
                           endcaps=["butt","arrow2"]);
        }

    // --- Curvature overlays ---
    // Non-zero κ: osculating circle (2-D) or disk in osculating plane (3-D).
    // Zero κ: black line segment through data point along local tangent.
    for (i = [0:1:np-1])
        if (!is_undef(eff_curv[i])) {
            kn = norm(eff_curv[i]);
            if (kn < 1e-12) {
                // Zero curvature — estimate tangent from nearest curve sample
                closest = min_index([for (p = curve) norm(p - points[i])]);
                t_idx   = max(1, min(len(curve)-2, closest));
                tang    = curve[t_idx+1] - curve[t_idx-1];
                tn      = norm(tang);
                if (tn > 1e-12)
                    color("black")
                        stroke([points[i] - tang/tn * arrow_len,
                                points[i] + tang/tn * arrow_len],
                               width=width/2);
            } else {
                // Non-zero curvature: κ_vec = κ·N̂, r = 1/κ, centre = pt + κ_vec/κ²
                r   = 1 / kn;
                ctr = points[i] + eff_curv[i] / (kn * kn);
                color([0,1,0,0.1])
                    if (is2d) {
                        translate(ctr) circle(r=r);
                    } else {
                        // Disk perpendicular to binormal B = T̂ × N̂
                        tn     = !is_undef(eff_deriv[i]) ? norm(eff_deriv[i]) : 0;
                        T_hat  = (tn > 1e-12) ? eff_deriv[i] / tn : [1,0,0];
                        N_hat  = eff_curv[i] / kn;
                        binom  = cross(T_hat, N_hat);
                        translate(ctr) rot(v=binom)
                            cylinder(h=width/2, r=r, center=true);
                    }
            }
        }
...
function nurbs_interp_surface(points, degree, method="centripetal", type="clamped",
    ...
    has_ve_pre && type_v == "closed" ?
        ...
        nurbs_interp_surface(new_pts, degree, method=method,
            type=[type_u, "clamped"],
