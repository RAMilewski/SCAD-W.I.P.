...
// edge = list of points in the coplanar boundary row/column.
...
//   normal1 = normal vector at a coplanar start edge (all points in the first row or first column lie in a single plane — e.g. a cone apex, a straight edge, or a flat polygonal boundary).  The code auto-detects whether the coplanar edge is u=0 (first row) or v=0 (first column) and fans the partial-derivative vectors outward from each edge point into the plane perpendicular to this normal.  Magnitude sets the derivative scale (same convention as *_deriv=).  Cannot be combined with the corresponding explicit *_deriv= parameter.  Default: undef
//   normal2 = normal vector at a coplanar end edge (last row or last column).  Auto-detects u=1 vs v=1.  Default: undef
...
function nurbs_interp_surface(points, degree, method="centripetal", type="clamped",
    ...
    let(
        ...
        // u=0: first row coplanar; v=0: first column coplanar; etc.
        start_u_degen = has_sn && _is_coplanar_pts(points[0]),
        start_v_degen = has_sn && _is_coplanar_pts([for (k = [0:1:n_rows-1]) points[k][0]]),
        end_u_degen   = has_en && _is_coplanar_pts(points[n_rows-1]),
        end_v_degen   = has_en && _is_coplanar_pts([for (k = [0:1:n_rows-1]) points[k][n_cols-1]]),
