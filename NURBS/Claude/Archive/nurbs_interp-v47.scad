...
// Returns true when x is an IEEE 754 NaN value.
// NaN is the only value where x != x is true; undef != undef is false.
function _is_nan(x) = !is_undef(x) && x != x;

// Returns true when a nurbs_interp() result contains multiple curve segments
// (produced by NaN corner markers in deriv=).  Corner results store the
// segment list at position [3]; regular results have undef there.
// A string in r[3] is a type override (e.g. "clamped" for closed-with-corners),
// not a segment list — use is_list() to distinguish.
function _nurbs_is_segmented(r) = is_list(r[3]);

// When a closed curve has corners, the result is a clamped B-spline with
// r[3] = "clamped" signaling the effective rendering type.
function _nurbs_eff_type(r, type) =
    is_string(r[3]) ? r[3] : type;
...
function _elevate_once_clamped(ctrl, p, xknots) =
    let(
        ...
        new_interior = len(interior) == 0 ? []
            : [for (i = [0:len(interior)-1]) each concat(
                  [interior[i]],
                  (i == len(interior)-1 ||
        ...
        C_vals = [for (u = grev)
            let(row = [for (j = [0:n_old]) _nip(j, p, u, U_old)])
            [for (d = [0:dim-1])
                sum([for (j = [0:n_old]) row[j] * ctrl[j][d]])]
        ],

        // Collocation matrix of the new basis at the Greville abscissae.
        A = [for (k = [0:n_new])
            [for (i = [0:n_new]) _nip(i, p_new, grev[k], U_new)]
        ],
...
// Synopsis: Raises the degree of a B-spline while preserving its shape exactly.
// Topics: NURBS Curves, Degree Elevation
// See Also: nurbs_interp(), nurbs_curve()
//
// Usage:
//   result = nurbs_elevate_degree(control, degree, knots, [type=], [times=]);
//
// Description:
//   Elevates the degree of a B-spline curve by `times` steps while
//   preserving its shape exactly.  The resulting curve is geometrically
//   identical to the original but represented with a higher degree and
//   more control points.  Each interior knot's multiplicity increases
//   by 1 per elevation step, so the continuity class at each knot is
//   preserved.
//   .
...
//
// Arguments:
//   control  = B-spline control points (any dimension >= 1)
//   degree   = current B-spline degree (>= 1)
...
//
// Returns:
//   [new_control, new_knots, new_degree]

function nurbs_elevate_degree(control, degree, knots,
                              type="clamped", times=1) =
    assert(type == "clamped",
...
//                 "dynamic" (per-chord dynamic exponent, Balta et al. 2020), or
//                 "foley" (deflection-angle correction, Foley & Neilson 1987).
//                 Default: "dynamic"
...
//                 Default: undef
//   corners     = list of interior point indices where C0 corner joints
//                 (sharp creases) should occur.  Equivalent to setting
//                 deriv[k]=0/0 at those indices.  Both clamped and closed.
//                 Default: undef
//
// Returns:
...
function nurbs_interp(points, degree, method="dynamic", type="clamped",
    ...
    assert(is_undef(corners) || (min(corners) >= 1 && max(corners) <= len(points)-2),
           str("nurbs_interp: corners= indices must be interior (1..", len(points)-2, ")"))
...
function _nurbs_interp_clamped(points, degree, method,
    ...
    let(
        ...
        nan_corners    = is_undef(eff_der) ? []
                       : [for (k = [0:n]) if (_is_nan(eff_der[k])) k],
        ...
        has_any_der  = !is_undef(eff_der) &&
                       len([for (k = [0:n])
                                if (!is_undef(eff_der[k]) && !_is_nan(eff_der[k])) k]) > 0,
    ...
    assert(bad_corner_end == [],
           str("nurbs_interp: NaN corner cannot be at the first or last point: ", bad_corner_end))
    assert(bad_corner_curv == [],
           str("nurbs_interp: curvature constraint cannot coincide with a NaN corner at: ", bad_corner_curv))
    assert(bad_curv_pts == [],
...
function _combine_corner_segs(segments, params, corner_idxs, p) =
    let(
        ...
        seg_sa  = [for (s = [0:n_segs-1]) s == 0         ? 0    : cpar[s-1]],
        seg_sb  = [for (s = [0:n_segs-1]) s == n_segs-1  ? 1    : cpar[s]  ],
        // Per-segment interior knots (exclude leading 0 and trailing 1),
        // remapped from local [0,1] to the segment's global interval.
        seg_gi  = [for (s = [0:n_segs-1])
            let(
                loc = [for (i = [1:len(segments[s][1])-2]) segments[s][1][i]],
                sa  = seg_sa[s],
                sb  = seg_sb[s]
            )
        ...
        // Combined control points: all of seg0, then seg[1:] for each later seg.
...
function _nurbs_interp_clamped_corners(points, p, method, eff_der, eff_curv, corner_idxs) =
    let(
        ...
        raw_segments = [for (s = [0:n_segs-1])
            let(
                i0       = seg_bounds[s],
                i1       = seg_bounds[s+1],
                seg_pts  = [for (k = [i0:i1]) points[k]],
                ...
                seg_der  = is_undef(eff_der) ? undef
                         : [for (k = [i0:i1])
                                _is_nan(eff_der[k]) ? undef : eff_der[k]],
                seg_curv = is_undef(eff_curv) ? undef
                         : [for (k = [i0:i1]) eff_curv[k]],
...
function _nurbs_interp_closed(points, degree, method, deriv, curv,
    ...
    let(
        ...
        nan_corners      = is_undef(deriv) ? []
                         : [for (k = [0:n-1]) if (_is_nan(deriv[k])) k],
        explicit_corners = is_undef(corners) ? [] : corners,
        corner_idxs      = deduplicate(sort(concat(nan_corners, explicit_corners))),
        has_corners      = len(corner_idxs) > 0,
        ...
        has_dl = !is_undef(deriv) &&
                 len([for (k = [0:n-1])
                          if (!is_undef(deriv[k]) && !_is_nan(deriv[k])) k]) > 0,
        ...
        bad_corner_curv = is_undef(curv) ? []
                        : [for (k = corner_idxs) if (!is_undef(curv[k])) k]
    )
    assert(bad_curv_pts == [],
           str("nurbs_interp: curvature constraint requires a derivative constraint ",
               "at the same point(s): ", bad_curv_pts))
    assert(bad_corner_curv == [],
           str("nurbs_interp: curvature constraint cannot coincide with a corner at: ",
               bad_corner_curv))
    has_corners
      ? _nurbs_interp_closed_corners(points, p, method, deriv, curv, corner_idxs)
      : (has_dl || has_cl)
...
function _nurbs_interp_closed_corners(points, p, method, deriv, curv,
                                       corner_idxs) =
    let(
        ...
        aug_pts = [for (k = [0:n-1]) points[(k + rot) % n],
                   points[rot]],

        // Remap remaining corners to rotated frame.
        rot_corners = sort([for (i = [1:len(corner_idxs)-1])
        ...
        aug_der = is_undef(deriv) ? undef :
            let(rd = [for (k = [0:n-1]) deriv[(k + rot) % n]],
                d0 = _is_nan(rd[0]) ? undef : rd[0])
            [d0, for (k = [1:n-1]) rd[k], d0],

        // Rotate and augment curv list.
        aug_curv = is_undef(curv) ? undef :
            let(rc = [for (k = [0:n-1]) curv[(k + rot) % n]])
            [rc[0], for (k = [1:n-1]) rc[k], rc[0]],
...
//              [curv=], [start_curv=], [end_curv=], [corners=]);
...
function nurbs_interp_curve(points, degree, splinesteps=16,
    ...
    let(
        result = nurbs_interp(points, degree, method=method,
                              ...
                              end_curv=end_curv, corners=corners)
    )
    _nurbs_is_segmented(result)
        ? let(
              segs  = result[3],
              paths = [for (seg = segs)
                           nurbs_curve(seg[0], seg[2], splinesteps=splinesteps,
                                       knots=seg[1], type="clamped")]
          )
          // Concatenate segments, dropping the duplicate shared corner point
          // at the start of each subsequent segment.
          [each paths[0],
           for (i = [1:len(paths)-1])
               for (j = [1:len(paths[i])-1]) paths[i][j]]
        : nurbs_curve(result[0], degree, splinesteps=splinesteps,
                      knots=result[1], type=_nurbs_eff_type(result, type));
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="dynamic",
    ...
    if (_nurbs_is_segmented(result)) {
        for (seg = result[3])
            debug_nurbs(seg[0], seg[2], splinesteps=splinesteps,
                        knots=seg[1], type="clamped", width=width, size=sz);
    } else {
        debug_nurbs(result[0], degree, splinesteps=splinesteps,
                    knots=result[1], type=_nurbs_eff_type(result, type),
                    width=width, size=sz);
    }
...
//
// Returns a list of [N_mat, xknots, seg_p, i0, i1] per segment.
...
function _build_edge_systems(params, p, edge_idxs) =
    ...
    [for (s = [0:n_segs-1])
        let(
            ...
            sys     = _build_interp_system(local_p, seg_p, "clamped")
        )
        [sys[0], sys[1], seg_p, i0, i1]
    ];
    ...
    let(
        raw_segments = [for (sys = systems)
            let(
                ...
                i1    = sys[4],
                seg_p = sys[2],
                seg_data = [for (k = [i0:i1]) data[k]],
                ctrl = linear_solve(N_mat, seg_data)
            )
            assert(ctrl != [],
...
//   Requires type="clamped" in the affected direction and cannot be
//   combined with boundary derivative constraints (*_der=, *_normal=,
//   flat_edges=) in the same direction.
//
// Arguments:
...
//                    Default: undef
//   start_normal   = normal vector at a degenerate start edge (all points in
//                    the first row or first column are the same — e.g. a cone
//                    apex).  The code auto-detects whether the degenerate edge
//                    is u=0 (first row) or v=0 (first column) and fans the
//                    partial-derivative vectors outward from the apex into the
//                    plane perpendicular to this normal.  Magnitude sets the
//                    derivative scale (same convention as *_der=).  Cannot be
//                    combined with the corresponding explicit *_der= parameter.
//                    Default: undef
//   end_normal     = normal vector at a degenerate end edge (last row or last
//                    column).  Auto-detects u=1 vs v=1.  Default: undef
//   flat_edges     = 4-element list [start_u, end_u, start_v, end_v] of scale
//                    factors for outward derivatives at each boundary edge.
//                    Each entry is a scalar (uniform) or a list (per-point,
//                    length must equal n_cols for u-edges, n_rows for v-edges).
//                    Set an entry to undef to leave that edge unconstrained.
//                    Requires coplanar boundary edges and type="clamped" in the
//                    affected direction.  Cannot be combined with explicit
//                    *_der= or *_normal= on the same edge.  Default: undef
//   u_edges        = list of interior row indices where C0 creases run in the
//                    v-direction.  Creates sharp edges across the surface at
//                    the specified rows.  Requires type_u="clamped".
//                    Incompatible with u-direction boundary derivatives.
//                    Default: undef
//   v_edges        = list of interior column indices where C0 creases run in
//                    the u-direction.  Creates sharp edges across the surface
//                    at the specified columns.  Requires type_v="clamped".
//                    Incompatible with v-direction boundary derivatives.
//                    Default: undef
//
// Returns:
...
function nurbs_interp_surface(points, degree, method="dynamic", type="clamped",
                              ...
                              start_normal=undef, end_normal=undef,
                              flat_edges=undef,
    ...
    let(
        ...
        has_sn  = !is_undef(start_normal),
        has_en  = !is_undef(end_normal),
        ...
        start_v_degen = has_sn && path_length([for (k = [0:n_rows-1]) points[k][0]]) < 1e-10,
        end_u_degen   = has_en && path_length(points[n_rows-1]) < 1e-10,
        end_v_degen   = has_en && path_length([for (k = [0:n_rows-1]) points[k][n_cols-1]]) < 1e-10,
        ...
        // Edge (C0 discontinuity) support.
        has_ue = !is_undef(u_edges) && len(u_edges) > 0,
        has_ve = !is_undef(v_edges) && len(v_edges) > 0
    )
    ...
    assert(!has_sn || (start_u_degen || start_v_degen),
           "nurbs_interp_surface: start_normal requires a degenerate start edge (first row or first column must be a single repeated point)")
    assert(!has_en || (end_u_degen || end_v_degen),
           "nurbs_interp_surface: end_normal requires a degenerate end edge (last row or last column must be a single repeated point)")
    assert(!has_sn || !(start_u_degen && start_v_degen),
           "nurbs_interp_surface: start_normal is ambiguous — both u=0 and v=0 edges are degenerate; use start_u_der or start_v_der explicitly")
    assert(!has_en || !(end_u_degen && end_v_degen),
           "nurbs_interp_surface: end_normal is ambiguous — both u=1 and v=1 edges are degenerate; use end_u_der or end_v_der explicitly")
    assert(!(has_sun && has_sud),
           "nurbs_interp_surface: start_normal resolves to u-direction but start_u_der was also given")
    assert(!(has_eun && has_eud),
           "nurbs_interp_surface: end_normal resolves to u-direction but end_u_der was also given")
    assert(!(has_svn && has_svd),
           "nurbs_interp_surface: start_normal resolves to v-direction but start_v_der was also given")
    assert(!(has_evn && has_evd),
           "nurbs_interp_surface: end_normal resolves to v-direction but end_v_der was also given")
    ...
    assert(!(has_fesu && has_sud),
           "nurbs_interp_surface: flat_edges[0] (start_u) conflicts with start_u_der")
    assert(!(has_feeu && has_eud),
           "nurbs_interp_surface: flat_edges[1] (end_u) conflicts with end_u_der")
    assert(!(has_fesv && has_svd),
           "nurbs_interp_surface: flat_edges[2] (start_v) conflicts with start_v_der")
    assert(!(has_feev && has_evd),
           "nurbs_interp_surface: flat_edges[3] (end_v) conflicts with end_v_der")
    assert(!(has_fesu && has_sun),
           "nurbs_interp_surface: flat_edges[0] (start_u) conflicts with start_normal on same edge")
    assert(!(has_feeu && has_eun),
           "nurbs_interp_surface: flat_edges[1] (end_u) conflicts with end_normal on same edge")
    assert(!(has_fesv && has_svn),
           "nurbs_interp_surface: flat_edges[2] (start_v) conflicts with start_normal on same edge")
    assert(!(has_feev && has_evn),
           "nurbs_interp_surface: flat_edges[3] (end_v) conflicts with end_normal on same edge")
    ...
    assert(!has_ue || (min(u_edges) >= 1 && max(u_edges) <= n_rows-2),
           str("nurbs_interp_surface: u_edges indices must be interior (1..", n_rows-2, ")"))
    assert(!has_ve || (min(v_edges) >= 1 && max(v_edges) <= n_cols-2),
           str("nurbs_interp_surface: v_edges indices must be interior (1..", n_cols-2, ")"))
    assert(!(has_ue && (has_sud || has_eud || has_sun || has_eun || has_fesu || has_feeu)),
           "nurbs_interp_surface: u_edges is incompatible with u-direction boundary derivatives/normals/flat_edges")
    assert(!(has_ve && (has_svd || has_evd || has_svn || has_evn || has_fesv || has_feev)),
           "nurbs_interp_surface: v_edges is incompatible with v-direction boundary derivatives/normals/flat_edges")
    let(
    ...
    assert(!has_fe || max(concat(
        [for (j = [0:n_cols-1]) abs((points[0][j]        - points[0][0]) * fe_N_hat)],
        [for (j = [0:n_cols-1]) abs((points[n_rows-1][j] - points[0][0]) * fe_N_hat)],
        [for (k = [0:n_rows-1]) abs((points[k][0]        - points[0][0]) * fe_N_hat)],
        [for (k = [0:n_rows-1]) abs((points[k][n_cols-1] - points[0][0]) * fe_N_hat)])) < 1e-6,
        "nurbs_interp_surface: flat_edges= requires all four boundary edges to be coplanar")
    let(
        // Compute effective derivative lists.
        // Priority: start_normal/end_normal (apex fan) > flat_edges > explicit *_der=.
        ...
        start_u_der_eff = has_sun
            ? _apex_tangents(start_normal, points[0][0], points[1])
            : has_fesu ? flat_su_der
            : start_u_der,
        end_u_der_eff   = has_eun
            ? [for (v = _apex_tangents(end_normal,
                                       points[n_rows-1][0],
                                       points[n_rows-2])) -v]
            : has_feeu ? flat_eu_der
            : end_u_der,
        start_v_der_eff = has_svn
            ? _apex_tangents(start_normal, points[0][0],
    ...
    let(
        ...
        v_edge_sys = has_ve
                   ? _build_edge_systems(v_params, p_v, v_edges) : undef,
        v_sys   = has_ve ? undef
        ...
        u_edge_sys = has_ue
                   ? _build_edge_systems(u_params, p_u, u_edges) : undef,
        u_sys   = has_ue ? undef
...
//             [start_normal=], [end_normal=], [flat_edges=],
//             [u_edges=], [v_edges=]);
//
// Description:
