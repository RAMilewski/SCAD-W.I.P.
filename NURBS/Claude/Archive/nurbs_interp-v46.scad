...
function _nurbs_is_segmented(r) = !is_undef(r[3]);
...
//   together and their indices are merged.  Requires type="clamped".
//
// Arguments:
...
//                 deriv[k]=0/0 at those indices.  Clamped only.
//                 Default: undef
//
// Returns:
...
function nurbs_interp(points, degree, method="dynamic", type="clamped",
    ...
    assert(type == "clamped" || is_undef(deriv) ||
               len([for (k = [0:len(points)-1]) if (_is_nan(deriv[k])) k]) == 0,
           "nurbs_interp: NaN corner markers in deriv= require type=\"clamped\"")
    ...
    assert(is_undef(corners) || type == "clamped",
           "nurbs_interp: corners= requires type=\"clamped\"")
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="dynamic",
    ...
    if (_nurbs_is_segmented(result)) {
        ...
        debug_nurbs(result[0], degree, splinesteps=splinesteps,
                    knots=result[1], type=type, width=width, size=sz);
    }
