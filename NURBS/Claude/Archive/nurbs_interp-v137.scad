...
// Entries are left in their original form (no dimension projection); callers
// that need dim-projected vectors must apply _force_deriv_dim() separately.
// NaN corner-marker entries (0/0) pass through unchanged.
// Returns undef when no constraint is specified.
function _merge_deriv_list(n, deriv, start_deriv=undef, end_deriv=undef) =
    !is_undef(deriv) ? deriv
...
// Entries may be signed scalars (2D) or vectors; no conversion is applied.
// Returns undef when no constraint is specified.
function _merge_curv_list(n, curvature, start_curvature=undef, end_curvature=undef) =
    !is_undef(curvature) ? curvature
...
function _nurbs_interp_clamped(points, degree, method,
    ...
    let(
        eff_der  = _merge_deriv_list(n, deriv, start_deriv, end_deriv),
        eff_curv = _merge_curv_list(n, curvature, start_curvature, end_curvature),
...
//   control polygon / knot rendering to `debug_nurbs()` and adds three overlays:
//   .
//   * **Derivative arrows** — black, half curve `width`, `endcap2="arrow2"`.
//     Drawn at every constrained data point.  Arrow length = derivative magnitude / n,
//     so relative magnitudes are preserved and the scale is independent of curve length.
//     Shown when `show_deriv=true`.
//   * **Curvature osculating circles / disks** — `color([0,1,0,0.1])`.
//     2-D: circle of radius 1/κ centred at the osculating centre.
//     3-D: thin disk (`cylinder`, `h=width/2`) in the osculating plane, oriented via
//     `rot(v = T̂ × N̂)`.  Accepts signed scalar κ (2D) or vector (any dim including
//     3D BOSL2 constants projected by `_force_deriv_dim()`).
//     Zero-curvature constraint: short green line segment through the data point
//     along the exact derivative direction (width = curve `width`).
//     Shown when `show_curvature=true`.
//   * **Data points** — magenta circle/sphere markers.
//   .
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="centripetal",
                          ...
                          show_control=false, show_knots=false, knots=undef,
    ...
    // Merge start_*/end_* into per-point lists, then apply _force_deriv_dim so
    // that BOSL2 3D direction constants (UP, LEFT, etc.) are projected correctly.
    eff_der_raw = _merge_deriv_list(np-1, deriv, start_deriv, end_deriv);
    eff_der     = is_undef(eff_der_raw) ? undef
                : [for (v = eff_der_raw)
                       is_undef(v) || is_nan(v) ? v : _force_deriv_dim(v, dim)];

    eff_curv = _merge_curv_list(np-1, curvature, start_curvature, end_curvature);
    ...
    if (show_curvature && !is_undef(eff_curv))
        for (i = [0:1:np-1])
            if (!is_undef(eff_curv[i])) {
                cv    = eff_curv[i];
                cv_d  = is_num(cv) ? undef : _force_deriv_dim(cv, dim);
                kn    = is_num(cv) ? abs(cv) : norm(cv_d);
                der_d = !is_undef(eff_der) && !is_undef(eff_der[i]) && !is_nan(eff_der[i])
                        ? eff_der[i] : undef;
                if (kn < 1e-12) {
                    // Zero curvature: short green segment along the exact derivative direction.
                    if (!is_undef(der_d) && norm(der_d) > 1e-12) {
                        T_hat = unit(der_d);
                        half  = norm(der_d) / np * 0.5;
                        color([0,1,0,0.1])
                    ...
                    // Non-zero curvature: osculating circle (2D) or disk (3D).
                    // N_hat points from the data point toward the centre of curvature.
                    N_hat = is_num(cv)
                        ? // Signed scalar (2D): rotate unit tangent by sign(cv) × 90°.
                          let(t_hat = !is_undef(der_d) && norm(der_d) > 1e-12
                                      ? unit(der_d) : [1, 0])
                          (cv >= 0) ? [-t_hat[1],  t_hat[0]]
                                    : [ t_hat[1], -t_hat[0]]
                        : // Vector: normalise after dim-projection.
                          cv_d / kn;
                    r   = 1 / kn;
                    ...
                    color([0,1,0,0.1])
                        if (is2d) {
                            ...
                            T_hat = !is_undef(der_d) && norm(der_d) > 1e-12
                                    ? unit(der_d) : [1, 0, 0];
