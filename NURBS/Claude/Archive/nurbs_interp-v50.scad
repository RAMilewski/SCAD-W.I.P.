...
//                 "foley" (centripetal + deflection-angle correction,
//                 Foley & Neilson 1987).  Default: "dynamic"
...
function nurbs_interp(points, degree, method="dynamic", type="clamped",
    ...
    assert(is_list(points[0]) && len(points[0]) >= 1,
           "nurbs_interp: data points must be vectors of dimension >= 1")
    assert(is_num(degree) && degree >= 1,
...
function _nurbs_interp_clamped(points, degree, method,
    ...
    let(
        ...
        nan_corners    = is_undef(eff_der) ? []
                       : [for (k = [0:n]) if (is_nan(eff_der[k])) k],
...
function _nurbs_interp_closed(points, degree, method, deriv, curv,
    ...
    let(
        ...
        nan_corners      = is_undef(deriv) ? []
                         : [for (k = [0:n-1]) if (is_nan(deriv[k])) k],
        explicit_corners = is_undef(corners) ? [] : corners,
        corner_idxs      = deduplicate(sort(concat(nan_corners, explicit_corners))),
        has_corners      = len(corner_idxs) > 0,
...
function _nurbs_interp_closed_constrained(points, p, method, eff_der, eff_curv) =
    let(
        ...
        // Seam rotation with span-collision validation (same as basic closed case).
        _rot    = _find_closed_rotation(points, n, p, method),
        _chk_rot = assert(!is_undef(_rot),
                          "nurbs_interp (closed+constrained): no valid seam rotation found; data may be too irregular."),
...
//                    or "foley" (centripetal + deflection-angle correction).
//                    Default: "dynamic"
...
function nurbs_interp_surface(points, degree, method="dynamic", type="clamped",
    ...
    assert(!has_fe || is_coplanar(concat(
        ...
        [for (k = [1:n_rows-2]) points[k][0]],
        [for (k = [1:n_rows-2]) points[k][n_cols-1]]), eps=1e-6),
