...
// Local rational quadratic interpolation (P&T §9.3.2/§9.3.3, α=1).
//
...
// the shoulder point for each segment is the intersection of the tangent
// lines from the two endpoints (α=1).  Weight w = cos(θ/2) where θ is
// the angle subtended at the shoulder point.
//
...
function _local_quadratic_interp(points, type) =
    ...
    let(
        ...
        // For each segment: compute shoulder point and weight (P&T §9.3.3, α=1).
        // S_k is the intersection of tangent lines from Q_k and Q_{k+1}.
        // Weight w = cos(θ/2) where θ is the angle at S_k.
        seg_data = [for (k = [0:1:n-1])
            ...
            // Parallel tangent lines: midpoint with w=1 (straight line).
            is_undef(hit)
                ? [(Qk + Qk1) / 2, 1]
                : let(
                    S     = hit[0],
