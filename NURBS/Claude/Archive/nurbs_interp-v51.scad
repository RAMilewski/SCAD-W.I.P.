...
// Synopsis: Raises the degree of a B-spline (not rational NURBS) while preserving its shape exactly.
// Topics: B-spline Curves, Degree Elevation
...
//   interpolation.
//   .
//   Currently supports "clamped" type only.  This function operates on
//   B-splines (uniform weights) only, not rational NURBS curves.
//
// Arguments:
...
//   ---
//   method = parameterization: "length", "centripetal", "dynamic", or "foley".  Default: "dynamic"
//   type = "clamped" or "closed".  Default: "clamped"
//   deriv = list of tangent vectors per data point; undef=unconstrained.  Scaled by chord length.  Default: undef
...
//   curvature = list of curvature constraints per data point; undef=unconstrained.  Each requires a deriv[k].  Default: undef
//   start_curvature = curvature at start point; shorthand for curvature[0].  Clamped only.  Default: undef
//   end_curvature = curvature at end point; shorthand for curvature[n].  Clamped only.  Default: undef
//   corners = list of interior point indices for C0 corners (sharp creases).  Default: undef
//
// Returns:
...
function _nurbs_interp_clamped(points, degree, method,
    ...
    let(
        ...
        bad_corner_curv = is_undef(eff_curv) ? []
                        : [for (k = corner_idxs) if (!is_undef(eff_curvature[k])) k],
        ...
        has_any_curv = !is_undef(eff_curv) &&
                       len([for (k = [0:n]) if (!is_undef(eff_curvature[k])) k]) > 0,
...
function _nurbs_interp_clamped_constrained(points, p, method, eff_der, eff_curv) =
    let(
        ...
        curv_specs = is_undef(eff_curv) ? []
                   : [for (k = [0:n]) if (!is_undef(eff_curvature[k]))
                          let(
                          ...
                          [k, _curv_to_d2(eff_curvature[k], tang_dir, dim, v2)]
                      ],
...
function _nurbs_interp_closed(points, degree, method, deriv, curvature,
    ...
    let(
        ...
        has_cl = !is_undef(curvature) &&
                 len([for (k = [0:n-1]) if (!is_undef(curvature[k])) k]) > 0,
...
function _nurbs_interp_closed_corners(points, p, method, deriv, curvature,
    ...
    let(
        ...
        aug_curv = is_undef(curvature) ? undef :
            let(rc = [for (k = [0:n-1]) curvature[(k + rot) % n]])
...
//   degree = B-spline degree: scalar or [u_degree, v_degree]
//   ---
//   method = parameterization: "length", "centripetal", "dynamic", or "foley".  Default: "dynamic"
//   type = "clamped"/"closed", or [u_type, v_type].  Default: "clamped"
//   u_edge1_deriv = list of n_cols ∂S/∂u vectors along u=0 boundary.  Clamped u only.  Default: undef
//   u_edge2_deriv = list of n_cols ∂S/∂u vectors along u=1 boundary.  Clamped u only.  Default: undef
//   v_edge1_deriv = list of n_rows ∂S/∂v vectors along v=0 boundary.  Clamped v only.  Default: undef
//   v_edge2_deriv = list of n_rows ∂S/∂v vectors along v=1 boundary.  Clamped v only.  Default: undef
//   normal1 = normal vector at degenerate start edge (apex).  Auto-detects u=0 vs v=0.  Default: undef
//   normal2 = normal vector at degenerate end edge (apex).  Auto-detects u=1 vs v=1.  Default: undef
//   flat_edges = [start_u, end_u, start_v, end_v] scale factors for flat outward derivatives.  Default: undef
//   u_edges = list of interior row indices for C0 creases in v-direction.  Default: undef
//   v_edges = list of interior column indices for C0 creases in u-direction.  Default: undef
//
// Returns:
...
function nurbs_interp_surface(points, degree, method="dynamic", type="clamped",
    ...
    assert(!has_fesu || min([for (j = [0:n_cols-1]) let(d = points[1][j] - points[0][j], d_flat = d - (d * fe_N_hat) * fe_N_hat) norm(d_flat)]) > 1e-10,
           "nurbs_interp_surface: flat_edges start_u direction is perpendicular to the boundary plane at one or more points")
    assert(!has_feeu || min([for (j = [0:n_cols-1]) let(d = points[n_rows-1][j] - points[n_rows-2][j], d_flat = d - (d * fe_N_hat) * fe_N_hat) norm(d_flat)]) > 1e-10,
           "nurbs_interp_surface: flat_edges end_u direction is perpendicular to the boundary plane at one or more points")
    assert(!has_fesv || min([for (k = [0:n_rows-1]) let(d = points[k][1] - points[k][0], d_flat = d - (d * fe_N_hat) * fe_N_hat) norm(d_flat)]) > 1e-10,
           "nurbs_interp_surface: flat_edges start_v direction is perpendicular to the boundary plane at one or more points")
    assert(!has_feev || min([for (k = [0:n_rows-1]) let(d = points[k][n_cols-1] - points[k][n_cols-2], d_flat = d - (d * fe_N_hat) * fe_N_hat) norm(d_flat)]) > 1e-10,
