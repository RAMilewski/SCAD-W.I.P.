...
// Check whether a list of points is collinear (all on one line).
// Returns true for n <= 2.  Relative tolerance: |cross|² < 1e-10 × span².
// Works for both 2D (cross returns scalar) and 3D (cross returns vector).

function _pts_collinear(pts) =
    let(
        n   = len(pts),
        d   = pts[n-1] - pts[0],
        dn2 = d * d
    ...
    n <= 2 || dn2 < 1e-20
    || len([for (i = [1:n-2])
                let(c = cross(pts[i] - pts[0], d))
                if (c * c > 1e-10 * dn2) i
           ]) == 0;


// Project vector v onto direction dir.
// Returns the component of v parallel to dir.
// Handles zero-length dir gracefully (returns zero vector).

function _proj_onto_dir(v, dir) =
    let(dn2 = dir * dir)
    dn2 < 1e-20 ? v * 0 : (v * dir) / dn2 * dir;
...
//   four boundary edges.  Each edge derivative takes a SINGLE vector
//   and requires the corresponding data edge to be collinear (a
//   straight line).  The surface lifts off the edge at the specified
//   angle in the interior; this is the correct model for G1 continuity
//   along a straight shared edge.
//   .
//   When only one pair of constraints is active (u-only or v-only) the
//   full derivative vector is applied uniformly via the partition of
//   unity identity, giving an exactly uniform cross-edge angle.
//   .
//   When BOTH u-constraints AND v-constraints are active simultaneously,
//   the four corner edges of the surface are collinear (flat).  Applying
//   the full derivative at a flat corner edge would introduce a component
//   perpendicular to the edge direction, which forces control-point
//   oscillation and makes the edge non-collinear.  To prevent this, the
//   derivative is projected onto the edge direction at the four boundary
//   rows/columns.  The full cross-edge angle is preserved for all
//   interior rows/columns; the corner transitions are smooth but have
//   a reduced cross-edge component at the corner points themselves.
//   .
...
//                 Default: "clamped"
//   start_u_der = single derivative vector for ∂S/∂u everywhere along the u=0
//                 boundary (first row edge).  Requires type_u="clamped" and that
//                 the first row be collinear.  Scaled by average u-direction chord
//                 length; pass a unit vector for natural arc-length speed.
//                 Default: undef
//   end_u_der   = single derivative vector for ∂S/∂u along the u=1 boundary
//                 (last row edge).  Requires last row collinear.  Default: undef
//   start_v_der = single derivative vector for ∂S/∂v everywhere along the v=0
//                 boundary (first column edge).  Requires type_v="clamped" and
//                 that the first column be collinear.  Scaled by average
//                 v-direction chord length.  Default: undef
//   end_v_der   = single derivative vector for ∂S/∂v along the v=1 boundary
//                 (last column edge).  Requires last column collinear.
//                 Default: undef
//
// Returns:
...
function nurbs_interp_surface(points, degree, method="dynamic", type="clamped",
    ...
    assert(!(has_sud || has_eud) || type_u == "clamped",
           "nurbs_interp_surface: start_u_der/end_u_der require type_u=\"clamped\"")
    assert(!(has_svd || has_evd) || type_v == "clamped",
           "nurbs_interp_surface: start_v_der/end_v_der require type_v=\"clamped\"")
    assert(!has_svd || _pts_collinear([for (k = [0:n_rows-1]) points[k][0]]),
           "nurbs_interp_surface: start_v_der requires the v=0 edge (first column) to be collinear")
    assert(!has_evd || _pts_collinear([for (k = [0:n_rows-1]) points[k][n_cols-1]]),
           "nurbs_interp_surface: end_v_der requires the v=1 edge (last column) to be collinear")
    assert(!has_sud || _pts_collinear(points[0]),
           "nurbs_interp_surface: start_u_der requires the u=0 edge (first row) to be collinear")
    assert(!has_eud || _pts_collinear(points[n_rows-1]),
           "nurbs_interp_surface: end_u_der requires the u=1 edge (last row) to be collinear")
    let(
        ...
        // Average v-direction path length across all rows.  The same scaled
        // derivative vector is applied to every row so that ∂S/∂v is uniform
        // along the constrained edge.  (Using avg rather than per-row ensures
        // the RHS is identical for all rows, which makes D_v[i] = d_v_scaled
        // for every u-control row i — exact via partition of unity.)
        avg_v_path_len = sum([for (k = [0:n_rows-1]) path_length(points[k])]) / n_rows,

        // Average u-direction path length across all columns.
        avg_u_path_len = sum([for (l = [0:n_cols-1])
                                  path_length([for (k = [0:n_rows-1]) points[k][l]])
                             ]) / n_cols,

        // Scaled boundary derivative vectors — one per edge, same for all rows/columns.
        d_svd = has_svd ? _force_deriv_dim(start_v_der, dim) * avg_v_path_len : undef,
        d_evd = has_evd ? _force_deriv_dim(end_v_der,   dim) * avg_v_path_len : undef,
        d_sud = has_sud ? _force_deriv_dim(start_u_der, dim) * avg_u_path_len : undef,
        d_eud = has_eud ? _force_deriv_dim(end_u_der,   dim) * avg_u_path_len : undef,

        // When u-constraints AND v-constraints are both active, the four boundary
        // rows/columns are collinear.  Applying a full 3D derivative to a flat boundary
        // row (in Pass 1) or a flat boundary column (in Pass 2) introduces a component
        // perpendicular to the edge direction.  This forces interior control points to
        // over-compensate, causing visible oscillation along the edge.
        //
        // Fix: for boundary rows k=0 and k=n_rows-1 (in Pass 1), and for boundary
        // columns j=0 and j=n_cols-1 (in Pass 2), project the derivative onto the
        // edge direction.  This removes the perpendicular component, keeping the edge
        // curve collinear.  The perpendicular (lift-off) component is still applied to
        // all interior rows/columns via the partition of unity argument, giving the
        // desired cross-edge angle in the interior of the boundary while the corners
        // remain flat.
        //
        // proj_row0/rown — project v-derivatives for boundary rows iff u-constraints
        // are also active (because u-constraints assert those rows are collinear).
        // proj_col0/coln — project u-derivatives for boundary cols iff v-constraints
        // are also active (because v-constraints assert those cols are collinear).
        proj_row0 = (has_svd || has_evd) && has_sud,
        proj_rown = (has_svd || has_evd) && has_eud,
        proj_col0 = (has_sud || has_eud) && has_svd,
        proj_coln = (has_sud || has_eud) && has_evd,
        // Raw edge direction vectors.  No normalization needed — _proj_onto_dir
        // uses (v·dir)/(dir·dir)*dir which is correct for any non-zero dir.
        row0_dir  = proj_row0 || proj_rown
                  ? points[0][n_cols-1]       - points[0][0]       : undef,
        rown_dir  = proj_row0 || proj_rown
                  ? points[n_rows-1][n_cols-1] - points[n_rows-1][0] : undef,
        col0_dir  = proj_col0 || proj_coln
                  ? points[n_rows-1][0]       - points[0][0]       : undef,
        coln_dir  = proj_col0 || proj_coln
                  ? points[n_rows-1][n_cols-1] - points[0][n_cols-1] : undef,

        // ----- Build v-direction system -----
        v_sys   = (has_svd || has_evd)
                ? _build_clamped_system_with_derivs(v_params, p_v, has_svd, has_evd)
                : _build_interp_system(v_params, p_v, type_v),
        N_v     = v_sys[0],
        v_knots = v_sys[1],

        // ----- Pass 1: Interpolate rows in v-direction -----
        // For interior rows, the full derivative RHS is used (partition of unity
        // guarantees uniform ∂S/∂v along the constrained column edge).
        // For boundary rows 0 and n_rows-1, project the derivative onto the row
        // edge direction to prevent perpendicular oscillation.
        R = [for (k = [0:n_rows-1])
            let(
                do_proj = (k == 0 && proj_row0) || (k == n_rows-1 && proj_rown),
                rd      = k == 0 ? row0_dir : rown_dir,
                svd_rhs = has_svd ? [do_proj ? _proj_onto_dir(d_svd, rd) : d_svd] : [],
                evd_rhs = has_evd ? [do_proj ? _proj_onto_dir(d_evd, rd) : d_evd] : [],
                rhs     = concat(points[k], svd_rhs, evd_rhs)
            )
        ...
        u_sys   = (has_sud || has_eud)
                ? _build_clamped_system_with_derivs(u_params, p_u, has_sud, has_eud)
                : _build_interp_system(u_params, p_u, type_u),
        N_u     = u_sys[0],
        u_knots = u_sys[1],

        // ----- Pass 2: Interpolate columns in u-direction -----
        // For interior columns, the full derivative RHS is used.
        // For boundary columns j=0 and j=n_cols-1, project the derivative onto
        // the column edge direction to preserve collinearity of the output edge.
        R_T  = [for (j = [0:n_v_ctrl-1])
                    [for (k = [0:n_rows-1]) R[k][j]]],
        P_T  = [for (j = [0:n_v_ctrl-1])
            let(
                do_proj = (j == 0 && proj_col0) || (j == n_cols-1 && proj_coln),
                cd      = j == 0 ? col0_dir : coln_dir,
                sud_rhs = has_sud ? [do_proj ? _proj_onto_dir(d_sud, cd) : d_sud] : [],
                eud_rhs = has_eud ? [do_proj ? _proj_onto_dir(d_eud, cd) : d_eud] : [],
                rhs     = concat(R_T[j], sud_rhs, eud_rhs)
            )
            linear_solve(N_u, rhs)
        ],

        // Transpose back to final control grid.
        n_u_ctrl = len(P_T[0]),
