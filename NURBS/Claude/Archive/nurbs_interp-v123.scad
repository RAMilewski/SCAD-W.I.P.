...
function _coplanar_inward_tangents(scales, edge, ring, periodic=false) =
    ...
    [for (j = [0:1:n-1])
        ...
        : let(
            ...
            // cross(P_hat, T3) = left-hand normal (inward for CCW polygon).
            // Multiply by sign to flip for CW polygons.
            in3    = sign * cross(P_hat, T3),
...
function nurbs_interp_surface(points, degree, method="centripetal", type="clamped",
    ...
    assert(fe1_ok,
           "nurbs_interp_surface: flat_end1 requires a coplanar non-collinear boundary edge (must lie in a 2D plane)")
    assert(fe2_ok,
           "nurbs_interp_surface: flat_end2 requires a coplanar non-collinear boundary edge (must lie in a 2D plane)")
