...
module debug_nurbs_interp(points, degree, splinesteps=16, method="centripetal",
    ...
    // --- Corner marks (NaN entries in eff_der) ---
    // 2D: rotated square stroke.  3D: octahedron wireframe.
    if (!is_undef(eff_der))
        for (i = [0:1:np-1])
            if (!is_undef(eff_der[i]) && is_nan(eff_der[i]))
                color("black")
                    translate(points[i])
                        if (is2d)
        ...
        for (i = [0:1:np-1])
            if (!is_undef(eff_curv[i])) {
                ...
                if (kn < 1e-12) {
                    ...
                    // perpendicular to T_hat (tangential component stripped).
                    N_hat = is_num(cv)
                        ? // Signed scalar (2D): rotate unit tangent 90° by sign.
                          (cv >= 0) ? [-T_hat[1],  T_hat[0]]
                                    : [ T_hat[1], -T_hat[0]]
                        : // Vector: project perpendicular to tangent, then normalise.
                          // This removes any tangential component and gives the true
                          // principal normal direction (in the T̂–N̂ osculating plane).
                          let(cv_perp = cv - (cv * T_hat) * T_hat)
                          unit(cv_perp);
                    r   = 1 / kn;
                    ...
                    color([0,1,0,0.1])
                        if (is2d) {
                            ...
                            // Cylinder axis = binormal B̂ = T̂ × N̂ (perpendicular to
                            // the osculating plane).  rot(v=B̂) aligns Z to B̂ so the
                            // circular cross-section lies in the T̂–N̂ plane.
