...
// directions are estimated using the five-point method (Eqs. 9.29 + 9.31);
// the intermediate
...
function _local_quadratic_interp(points, type) =
    ...
    let(
        ...
        // P&T §9.3.3, Eq. (9.44): R_k = (1-α)*M_k + α*S_k,  w_k = cos(α*θ_k/2)
        // where S_k is the shoulder (tangent-line intersection), M_k is the chord
        // midpoint, θ_k is the angle at S_k, and α_k=1 when both ray parameters
        // are positive.  When the shoulder falls behind a data point (s≤0 or t≤0),
        // α is reduced to the largest value that keeps the Bézier tangent
        // directions correct at both segment endpoints.
        seg_data = [for (k = [0:1:n-1])
            ...
            // Parallel tangent lines: midpoint with w=1 (parabola / straight line).
            is_undef(hit)
                ? [(Qk + Qk1) / 2, 1]
                : let(
                    S     = hit[0],
                    ...
                    t_val = hit[2],
                    M     = (Qk + Qk1) / 2,
                    // Angle θ at the shoulder point S.
                    v1    = Qk  - S,
                    v2    = Qk1 - S,
                    denom = max(norm(v1) * norm(v2), 1e-15),
                    cos_t = max(min((v1 * v2) / denom, 1), -1),
                    theta = acos(cos_t),    // degrees (OpenSCAD convention)
                    // Adaptive α: full 1 when both s,t > 0; otherwise reduce
                    // so that (R−Qk)·Tk > 0 and (Qk1−R)·Tk1 > 0.
                    chord = Qk1 - Qk,
                    c_s   = (chord * Tk) / max(2 * (Tk * Tk), 1e-15),
                    c_e   = (chord * Tk1) / max(2 * (Tk1 * Tk1), 1e-15),
                    a_s   = s_val > 0 ? 1 : (c_s > 1e-10 ? c_s / (c_s - s_val) : 0),
                    a_e   = t_val > 0 ? 1 : (c_e > 1e-10 ? c_e / (c_e - t_val) : 0),
                    alpha = max(0, min(1, 0.9 * min(a_s, a_e))),
                    R     = (1 - alpha) * M + alpha * S,
                    w     = cos(alpha * theta / 2)
                  )
                  [R, w]
