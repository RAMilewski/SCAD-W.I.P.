...
function _dynamic_dists(raw, emin=0.35, emax=0.65) =
    ...
    // Divide each chord by cmax before applying the exponent so the
    // parameterization is scale-invariant: pow(d/cmax, e) is unchanged when
    // all chords are scaled by any constant factor.
    log_r < 1e-12
      ? [for (d = raw) sqrt(d / cmax)]   // equal chords → uniform spacing
      : [for (d = raw)
...
function nurbs_interp_surface(points, degree, method="dynamic",
                              type="clamped") =
    let(
    ...
    let(
        ...
        u_params = _surface_params_u(points, method,
                                     type_u == "closed"),
        v_params = _surface_params_v(points, method,
                                     type_v == "closed"),
...
//       [method=], [type=], [style=], [data_color=], [data_size=]);
