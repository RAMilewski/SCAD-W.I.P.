...
//
// When every segment achieves the full degree p, the segments are assembled
// into a single clamped B-spline via _combine_corner_segs() and a regular
// [control, knots, 0] result is returned.  Otherwise (some segment needed
// degree reduction) a segmented result [undef, undef, 0, segments] is
// returned; callers use _nurbs_is_segmented() to detect this and read
// seg[2] for the per-segment degree.
...
function _nurbs_interp_clamped_corners(points, p, method, eff_der, eff_curv, corner_idxs) =
    let(
        ...
        // If every segment achieved degree p, combine into one B-spline.
        all_full = !any([for (seg = segments) seg[2] != p])
    )
    all_full
    ? _combine_corner_segs(segments, params, corner_idxs, p)
    : [undef, undef, 0, segments];
