...
//   each corner.  `extra_pts=` is then distributed across the eligible segments
//   proportionally to their size (longer segments receive more extra control points).
//   Degree-reduced segments (those with fewer than `degree + 1` points) are not
//   eligible and receive no extra control points.
//
// Arguments:
