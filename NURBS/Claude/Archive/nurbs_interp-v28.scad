...
function nurbs_interp(points, degree, param="dynamic", type="clamped",
    ...
    assert(param == "length" || param == "centripetal" || param == "dynamic" || param == "fang",
           str("nurbs_interp: param must be \"length\", \"centripetal\", \"dynamic\", or \"fang\", got \"", param, "\""))
...
//
// start_der= and end_der= are convenience shorthands for deriv= at the
// endpoints.  They are merged into a deriv list here so that all
...
function _nurbs_interp_clamped(points, degree, param,
    ...
    let(
        ...
        // Convert start_der / end_der to a deriv list when no explicit
        // deriv= was provided.
    ...
    has_any ? _nurbs_interp_clamped_derivlist(points, p, param, eff_der)
...
function _nurbs_interp_clamped_derivlist(points, p, param, deriv) =
    let(
        n         = len(points) - 1,
        dim       = len(points[0]),
        ...
        der_specs = [for (k = [0:n]) if (!is_undef(deriv[k]))
                        [k, _force_deriv_dim(deriv[k], dim) * path_len]],
