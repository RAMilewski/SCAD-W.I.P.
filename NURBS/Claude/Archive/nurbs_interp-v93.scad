...
// Quantile-resample N data parameters to M virtual parameters.
//
// For M > N, distributes extra DOFs where data density is high by
// linearly interpolating the sorted data parameters at uniform
// quantile positions.  For M = N, returns the original parameters.
//
// params:  N sorted data parameters.
//          Closed: [0, ...) in [0, 1).  Clamped: [0, ..., 1].
// M:       target number of virtual parameters (>= N).
// closed:  true for periodic [0, 1), false for clamped [0, 1].
//
// Returns M sorted virtual parameters in the same domain as params.

function _quantile_resample(params, M, closed=false) =
    let(N = len(params))
    M == N ? params
    : closed
        // Closed: N params in [0, 1), period wraps at 1.
        ? [for (j = [0:1:M-1])
            let(
                idx    = j * N / M,
                i      = floor(idx),
                f      = idx - i,
                u_next = i < N - 1 ? params[i + 1] : 1
            )
            params[i] + f * (u_next - params[i])
          ]
        // Clamped: N params in [0, 1], endpoints fixed.
        : [for (j = [0:1:M-1])
            let(
                idx = j * (N - 1) / (M - 1),
                i   = min(floor(idx), N - 2),
            ...
            params[i] + f * (params[i + 1] - params[i])
          ];
...
function _closed_constrained_solve(points, p, method, eff_der, eff_curv, rot) =
    let(
        ...
        // Build M bar_knots from data-density-aware quantile resampling.
        // Resample N data params to M virtual params at uniform quantiles,
        // then apply standard periodic knot averaging.  Extra DOFs land
        // where data is dense, not at constraint locations.
        virtual_params = _quantile_resample(raw_params, M, closed=true),
        aug_bar_raw    = _avg_knots_periodic(virtual_params, p)[0],
...
function _build_clamped_system_with_derivs(params, p, has_sd, has_ed) =
    let(
        ...
        N_ctrl  = n + 1 + n_extra,
        // Quantile-resample n+1 data params to N_ctrl virtual params,
        // then compute interior knots via standard averaging.  This
        // distributes extra DOFs by data density rather than clustering
        // them at boundaries.
        virtual     = _quantile_resample(params, N_ctrl, closed=false),
        int_kn      = _avg_knots_interior(virtual, p),
        U_full      = _full_clamped_knots(int_kn, p),
        interp_rows = [for (k = [0:1:n])
