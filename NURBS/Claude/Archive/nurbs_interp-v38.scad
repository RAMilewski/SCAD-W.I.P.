...
function nurbs_interp_surface(points, degree, method="dynamic", type="clamped",
    ...
    assert(!has_sun || path_length(points[0]) < 1e-10,
           "nurbs_interp_surface: start_u_normal requires the u=0 edge (first row) to be a single degenerate point")
    assert(!has_eun || path_length(points[n_rows-1]) < 1e-10,
           "nurbs_interp_surface: end_u_normal requires the u=1 edge (last row) to be a single degenerate point")
    assert(!has_svn || path_length([for (k = [0:n_rows-1]) points[k][0]]) < 1e-10,
           "nurbs_interp_surface: start_v_normal requires the v=0 edge (first column) to be a single degenerate point")
    assert(!has_evn || path_length([for (k = [0:n_rows-1]) points[k][n_cols-1]]) < 1e-10,
           "nurbs_interp_surface: end_v_normal requires the v=1 edge (last column) to be a single degenerate point")
    let(
