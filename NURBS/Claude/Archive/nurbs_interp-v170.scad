...
function _elevate_once_closed(ctrl, p, bar_knots, curr_mult=undef) =
    let(
        ...
        // Skip sites below a_new (they correspond to the periodic extension, not the
        // active domain); instead, start from the first site ≥ a_new and take the
        // next n_new consecutive sites.  Sites that drift above b_new are wrapped
        // back into [a_new, b_new) by subtracting T.
