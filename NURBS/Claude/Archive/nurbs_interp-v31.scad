...
function nurbs_interp(points, degree, param="dynamic", type="clamped",
    ...
    type == "clamped" ? _nurbs_interp_clamped(points, degree, param,
                                               deriv, start_der, end_der,
                                               curv, start_curv, end_curv)
  :                     _nurbs_interp_closed(points, degree, param, deriv, curv);
...
function _nurbs_interp_closed(points, degree, param, deriv, curv) =
    ...
    assert(bad_curv_pts == [],
           str("nurbs_interp: curvature constraint requires a derivative constraint ",
               "at the same point(s): ", bad_curv_pts))
    (has_dl || has_cl)
      ? _nurbs_interp_closed_constrained(points, p, param, deriv, curv)
...
function _nurbs_interp_closed_constrained(points, p, param, eff_der, eff_curv) =
    let(
        n         = len(points),
        dim       = len(points[0]),
