...
//                [row_edges=], [col_edges=], [extra_pts=], [smooth=], [first_row_deriv=], [last_row_deriv=], [first_col_deriv=], [last_col_deriv=]);
...
//   .
//   **Crease edges** — `row_edges=` and `col_edges=` insert C0 discontinuities across the
//   surface.  `row_edges=` specifies the indices of rows that will be edges or creases,
//   and `col_edges=` specifies the indices of columns that will be edges or creases. 
...
//   row_edges   = Row indidces (or index) of rows that are treated as edges or creases.  
