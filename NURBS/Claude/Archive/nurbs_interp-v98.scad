...
//   smooth = regularization order for extra_pts: 1 = first-difference penalty (minimizes control polygon length/variation), 2 = second-difference penalty (minimizes control polygon bending).  Default: 2
//
// Returns:
...
function nurbs_interp(points, degree, method="centripetal", type="clamped",
    ...
    assert(smooth == 1 || smooth == 2,
           str("nurbs_interp: smooth must be 1 (first-difference) or 2 (second-difference), got ", smooth))
    let(
        raw = type == "clamped"
...
function _nurbs_interp_clamped_constrained(points, p, method, eff_der, eff_curv,
    ...
    // When extra_pts == 0, system is square: solve A*P = rhs directly.
    // When extra_pts > 0, system is underdetermined (M > N_rows):
    // solve via KKT system: min ||L*P||² s.t. A*P = rhs.
    extra_pts == 0
    ? let(
        control = linear_solve(A_constr, rhs_constr)
      )
      assert(control != [],
    ...
    : let(
        ...
        // [L^T*L  A^T] [P]   [  0  ]
        // [A      0  ] [Λ] = [rhs  ]
        // Size: (M + N_rows) × (M + N_rows)
        A_T = transpose(A_constr),
        LTL = [for (i = [0:1:M-1]) _ltl_row(M, i, smooth)],
        K   = [for (i = [0:1:M + N_rows - 1])
                   i < M
                   ? [each LTL[i], each A_T[i]]
                   : [each A_constr[i - M],
                      each [for (j = [0:1:N_rows-1]) 0]]
              ],
        kkt_rhs = [each [for (i = [0:1:M-1]) repeat(0, dim)],
                   each rhs_constr],
        sol     = linear_solve(K, kkt_rhs)
      )
      assert(sol != [],
             "nurbs_interp (clamped+constrained+KKT): singular system")
      // Extract control points from first M rows of solution.
      let(control = [for (i = [0:1:M-1]) sol[i]])
      [control, knots, 0];
...
function _closed_constrained_solve(points, p, method, eff_der, eff_curv, rot,
    ...
    // When extra_pts == 0, system is square: solve A*P = rhs directly.
    // When extra_pts > 0, solve via KKT with periodic L^T*L regularizer.
    extra_pts == 0
    ? let(control = linear_solve(A_constr, rhs_constr))
      control == [] ? undef : [control, aug_bar, rot]
    : let(
        A_T = transpose(A_constr),
        LTL = [for (i = [0:1:M-1]) _ltl_row(M, i, smooth, periodic=true)],
        K   = [for (i = [0:1:M + N_rows - 1])
                   i < M
                   ? [each LTL[i], each A_T[i]]
                   : [each A_constr[i - M],
