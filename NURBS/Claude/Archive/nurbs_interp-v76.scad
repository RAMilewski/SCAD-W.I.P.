...
// Cross product magnitude (dimension-agnostic).
function _cross_mag(a, b) =
    len(a) == 2 ? abs(a[0]*b[1] - a[1]*b[0])
                : norm(cross(a, b));

// Five-point tangent estimation (P&T §9.3.1, Eqs. 9.29 + 9.31).
// α_k = |q_{k-1} × q_k| / (|q_{k-1} × q_k| + |q_{k+1} × q_{k+2}|)
// V_k = (1 − α_k) q_k + α_k q_{k+1},   T_k = V_k / |V_k|
// Boundary extension via virtual chords (Eq 9.33).
//
// Returns [T_left, T_right] pairs per point.  At true corners
// (denom=0: both sides collinear), T_left = unit(q_k) (incoming)
// and T_right = unit(q_{k+1}) (outgoing), creating C0 discontinuity.
// At all other points T_left = T_right = unit(V_k).
function _five_point_tangents_lr(points) =
    let(
        n = len(points) - 1
    )
    n <= 0 ? [[points[0] * 0, points[0] * 0]]
  : let(
        // Chord vectors: q[k] = Q_k − Q_{k-1},  k = 1..n
        q = [for (k = [1:1:n]) points[k] - points[k-1]],
        // Virtual chords for boundary extension (Eq 9.33)
        q0  = n >= 2 ? 2*q[0] - q[1]         : q[0],
        qm1 = n >= 2 ? 2*q0 - q[0]           : q[0],
        qn1 = n >= 2 ? 2*q[n-1] - q[n-2]     : q[n-1],
        qn2 = n >= 2 ? 2*qn1 - q[n-1]        : q[n-1],
        // Extended array offset by 1: q_ext[j] = book's q[j−1]
        q_ext = [qm1, q0, each q, qn1, qn2]
    )
    [for (k = [0:1:n])
        let(
            qkm1 = q_ext[k],
            ...
            qk2  = q_ext[k+3],
            // Eq 9.31
            cross_l = _cross_mag(qkm1, qk),
            cross_r = _cross_mag(qk1, qk2),
            denom   = cross_l + cross_r,
            is_corner = denom < 1e-15,
            alpha   = is_corner ? 1 : cross_l / denom,
            // Eq 9.29
            V  = (1 - alpha) * qk + alpha * qk1,
            nV = norm(V),
            T  = nV < 1e-15 ? qk / max(norm(qk), 1e-15) : V / nV,
            // True corner: separate left/right tangents
            T_left  = is_corner ? qk  / max(norm(qk),  1e-15) : T,
            T_right = is_corner ? qk1 / max(norm(qk1), 1e-15) : T
        )
        [T_left, T_right]
    ];


// Shoulder-point weight from the triangle Q_{k-1}, R_k, Q_k (P&T §9.3.3).
// Returns cos(θ/2) where θ is the angle at R_k.
function _shoulder_weight(A, R, B) =
    let(
        v1 = A - R,
        v2 = B - R,
        d  = max(norm(v1) * norm(v2), 1e-15),
        ct = max(min((v1 * v2) / d, 1), -1)
    )
    cos(acos(ct) / 2);

// Local rational quadratic interpolation (P&T §9.3.2/§9.3.3).
//
// Given data points, computes a degree-2 rational B-spline where each
// pair of consecutive data points is connected by one or two conic arcs.
// Tangent directions are estimated using the five-point method
// (Eqs. 9.29 + 9.31) with left/right tangents at true corners.
//
// Normal segments: the shoulder R_k is the tangent-line intersection
// (Eq. 9.34 satisfied: γ_{k-1}>0, γ_k<0).  Weight w = cos(θ/2).
//
// Corner segments: tangent lines are parallel or the intersection
// violates Eq. 9.34.  The segment is split into two sub-arcs via
// Eqs. 9.41–9.42 with rational γ from Eq. 9.45 (α=2/3).
//
...
function _local_quadratic_interp(points, type) =
    ...
    let(
        ...
        TLR    = _five_point_tangents_lr(points),
        ALPHA  = 2/3,     // P&T recommended blending constant
        // Per-segment classification and control data.
        // Each segment uses T_right at start, T_left at end.
        // Normal: ["n", R, w]
        // Split:  ["s", R_A, w_A, Q_prime, R_B, w_B]
        seg_data = [for (k = [0:1:n-1])
            let(
                A         = points[k],
                B         = points[k+1],
                TA        = TLR[k][1],       // T_right at start
                TB        = TLR[k+1][0],     // T_left at end
                chord     = B - A,
                chord_len = norm(chord),
                chord_dir = chord_len > 1e-15 ? chord / chord_len : chord,
                hit       = _line_intersect(A, TA, B, -TB)
            )
            // Normal case: intersection exists with s>0, t>0 (Eq 9.34).
            (!is_undef(hit) && hit[1] > 1e-10 && hit[2] > 1e-10)
            ? ["n", hit[0], _shoulder_weight(A, hit[0], B)]
            : let(
                // Check collinear: both tangents parallel to chord → straight line.
                cross_A = _cross_mag(TA, chord_dir),
                cross_B = _cross_mag(TB, chord_dir)
              )
              (cross_A < 1e-10 && cross_B < 1e-10)
              ? ["n", (A + B) / 2, 1]
              // Split case: two sub-arcs (Eqs. 9.41–9.42, 9.45).
              : let(
                    cos_tA  = min(abs(TA * chord_dir), 1),
                    cos_tB  = min(abs(TB * chord_dir), 1),
                    den_A   = max(1 + ALPHA * cos_tA + (1 - ALPHA) * cos_tB, 1e-15),
                    den_B   = max(1 + (1 - ALPHA) * cos_tA + ALPHA * cos_tB, 1e-15),
                    gamma_A = chord_len / (2 * den_A),
                    gamma_B = chord_len / (2 * den_B),
                    RA      = A + gamma_A * TA,
                    RB      = B - gamma_B * TB,
                    Qp      = (gamma_A * RB + gamma_B * RA) / (gamma_A + gamma_B),
                    wa      = _shoulder_weight(A, RA, Qp),
                    wb      = _shoulder_weight(Qp, RB, B)
                ) ["s", RA, wa, Qp, RB, wb]
        ],
        ...
        ctrl = [points[0],
                for (k = [0:1:n-1]) each (
                    seg_data[k][0] == "n"
                    ? [seg_data[k][1], points[k+1]]
                    : [seg_data[k][1], seg_data[k][3],
                       seg_data[k][4], points[k+1]]
                )],
        // Assemble weights.
        wts = [1,
               for (k = [0:1:n-1]) each (
                   seg_data[k][0] == "n"
                   ? [seg_data[k][2], 1]
                   : [seg_data[k][2], 1, seg_data[k][5], 1]
               )],
        // Knot break parameters: interior data points plus split midpoints.
        // Each break parameter appears twice (double knot → C0).
        breaks = [for (k = [0:1:n-1]) each (
            let(is_split = seg_data[k][0] == "s")
            k < n-1
            ? (is_split
               ? [(params[k] + params[k+1]) / 2, params[k+1]]
               : [params[k+1]])
            : (is_split
...
// Workaround for BOSL2 nurbs_curve() bug: when weights= is defined,
// the recursive call drops the knots= parameter, so custom knots are
// silently ignored for rational NURBS.  This helper does the
// homogeneous conversion manually and calls nurbs_curve() without
// weights=, preserving the knots.

function _nurbs_curve_rational(ctrl, degree, wts, splinesteps, knots, type) =
    let(
        dim  = len(ctrl[0]),
        homo = [for (i = [0:1:len(ctrl)-1])
                    [each ctrl[i] * wts[i], wts[i]]],
        curve_homo = nurbs_curve(homo, degree, splinesteps=splinesteps,
                                 knots=knots, type=type)
    )
    [for (p = curve_homo)
        [for (j = [0:1:dim-1]) p[j] / p[dim]]];
...
function nurbs_interp_curve(points, degree, splinesteps=16,
    ...
    is_undef(result[4])
      ...
      : _nurbs_curve_rational(result[2], result[1], result[4],
                              splinesteps, result[3], result[0]);
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="dynamic",
    ...
    // BOSL2 bug workaround: nurbs_curve() drops knots= for rational
    // NURBS (non-undef weights).  Use _nurbs_curve_rational() instead.
    rational = !is_undef(result[4]);
    curve = rational
          ? _nurbs_curve_rational(result[2], result[1], result[4],
                                  splinesteps, result[3], result[0])
          : nurbs_curve(result[2], result[1], splinesteps=splinesteps,
    ...
    if (show_ctrl && !rational) {
        debug_nurbs(result[2], result[1], splinesteps=splinesteps,
                    knots=result[3], type=result[0],
                    width=width, size=sz);
    } else if (show_ctrl) {
        // Rational: render curve + control polygon manually
        // (debug_nurbs has the same BOSL2 knots-dropping bug).
        stroke(curve, width=width, closed=(result[0]=="closed"));
        stroke(result[2], width=width/2, color="lightblue",
               closed=(result[0]=="closed"));
        color("blue")
            for (i = [0:1:len(result[2])-1])
                translate(result[2][i])
                    if (len(result[2][i]) == 2) text(str(i), size=sz, anchor=CENTER);
                    else rot($vpr) text3d(str(i), size=sz, anchor=CENTER);
    } else {
        stroke(curve, width=width, closed=(result[0]=="closed"));
    }
