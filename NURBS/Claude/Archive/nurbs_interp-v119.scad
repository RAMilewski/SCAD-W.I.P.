...
//   _apex_fan_tangents(scales, apex, ring)
//   Fans outward from apex toward ring[j], projected perpendicular to the ring's
//   plane normal (auto-computed).  scales[j] sets magnitude; positive = outward.
//
// Coplanar edge (boundary points coplanar, not all identical):
//   _coplanar_inward_tangents(scales, edge, ring)
//   At each edge point, computes a unit vector perpendicular to the edge tangent
//   lying in the edge's plane, oriented toward the polygon centroid (= inward).
//   scales[j] sets magnitude; positive = inward (closing), negative = outward (flaring).
//   Falls back to ring direction when centroid is ambiguous (collinear edge points).
//
// Pass the negated result for an end (u=1 or v=1) edge; see caller.

function _apex_fan_tangents(scales, apex, ring) =
    let(
        axis  = _pts_plane_normal(ring),
        dim   = len(ring[0]),
        zero  = [for (i = [0:1:dim-1]) 0]
    )
    is_undef(axis) ? [for (r = ring) zero]
    : let(A_hat = axis / norm(axis))
    [for (j = [0:1:len(ring)-1])
        let(
            d      = ring[j] - apex,
            d3     = dim == 2 ? [d[0], d[1], 0] : d,
            dp3    = d3 - (d3 * A_hat) * A_hat,
            np     = norm(dp3),
            dp_n3  = np > 1e-12 ? dp3 / np : [0, 0, 0],
            dp     = dim == 2 ? [dp_n3[0], dp_n3[1]] : dp_n3
        )
        scales[j] * dp
    ];


function _coplanar_inward_tangents(scales, edge, ring) =
    let(
        n     = len(edge),
        dim   = len(edge[0]),
    ...
    [for (j = [0:1:n-1])
        ...
        : let(
            ...
            // Prefer centroid for orientation; fall back to ring when ambiguous
            C_dot = cand * (C - edge[j]),
...
//   normal1 = per-point scale array (or single scalar) for a coplanar start edge — all points in the first row or first column must lie in a single plane (or all be the same point).  The code auto-detects whether the coplanar edge is u=0 (first row) or v=0 (first column).  Apex sub-case (all identical): scale fans outward from the apex; positive = outward.  Coplanar sub-case: positive scale closes the surface inward (toward the polygon centroid), negative flares it outward.  A scalar is broadcast to all edge points.  Array length must equal n_cols for a u=0 edge or n_rows for a v=0 edge.  Cannot be combined with the corresponding explicit *_deriv= parameter.  Default: undef
//   normal2 = per-point scale array (or scalar) for a coplanar end edge (last row or last column).  Auto-detects u=1 vs v=1.  Same sign conventions as normal1.  Default: undef
...
function nurbs_interp_surface(points, degree, method="centripetal", type="clamped",
    ...
    let(
        ...
        // Broadcast scalar normal1/normal2 to per-point scale arrays.
        // n1_u / n2_u: one entry per column (for u=0 and u=1 edges, n_cols points).
        // n1_v / n2_v: one entry per row    (for v=0 and v=1 edges, n_rows points).
        n1_u = !has_sn ? undef
             : is_num(normal1) ? [for (i = [0:1:n_cols-1]) normal1] : normal1,
        n1_v = !has_sn ? undef
             : is_num(normal1) ? [for (i = [0:1:n_rows-1]) normal1] : normal1,
        n2_u = !has_en ? undef
             : is_num(normal2) ? [for (i = [0:1:n_cols-1]) normal2] : normal2,
        n2_v = !has_en ? undef
             : is_num(normal2) ? [for (i = [0:1:n_rows-1]) normal2] : normal2,
        // Auto-detect which parametric direction has the coplanar boundary edge.
        // Apex sub-case: all boundary points identical (cone tip).
        // Coplanar sub-case: all boundary points in a plane, not all identical.
        ...
        start_u_degen = has_sn && (start_u_apex || _is_coplanar_pts(points[0])),
        start_v_degen = has_sn && (start_v_apex || _is_coplanar_pts([for (k = [0:1:n_rows-1]) points[k][0]])),
        end_u_degen   = has_en && (end_u_apex   || _is_coplanar_pts(points[n_rows-1])),
        end_v_degen   = has_en && (end_v_apex   || _is_coplanar_pts([for (k = [0:1:n_rows-1]) points[k][n_cols-1]])),
    ...
    assert(!has_sn || (start_u_degen || start_v_degen),
           "nurbs_interp_surface: normal1 requires a coplanar start edge (first row or first column must be coplanar)")
    assert(!has_en || (end_u_degen || end_v_degen),
           "nurbs_interp_surface: normal2 requires a coplanar end edge (last row or last column must be coplanar)")
    assert(!has_sn || !(start_u_degen && start_v_degen),
           "nurbs_interp_surface: normal1 is ambiguous — both u=0 and v=0 edges are coplanar; use u_edge1_deriv or v_edge1_deriv explicitly")
    assert(!has_en || !(end_u_degen && end_v_degen),
           "nurbs_interp_surface: normal2 is ambiguous — both u=1 and v=1 edges are coplanar; use u_edge2_deriv or v_edge2_deriv explicitly")
    ...
    assert(!has_sun || is_num(normal1) || len(normal1) == n_cols,
           str("nurbs_interp_surface: normal1 (u=0 edge) must be a scalar or list of ", n_cols, " values"))
    assert(!has_svn || is_num(normal1) || len(normal1) == n_rows,
           str("nurbs_interp_surface: normal1 (v=0 edge) must be a scalar or list of ", n_rows, " values"))
    assert(!has_eun || is_num(normal2) || len(normal2) == n_cols,
           str("nurbs_interp_surface: normal2 (u=1 edge) must be a scalar or list of ", n_cols, " values"))
    assert(!has_evn || is_num(normal2) || len(normal2) == n_rows,
           str("nurbs_interp_surface: normal2 (v=1 edge) must be a scalar or list of ", n_rows, " values"))
    ...
    let(
        ...
        // Priority: normal1/normal2 > flat_edges > explicit *_der=.
        // Apex edge (all boundary points identical): fan outward from apex, axis
        //   auto-computed from ring plane normal.  Positive scale = outward.
        // Coplanar edge: inward direction = toward polygon centroid (perpendicular
        //   to edge tangent, in the edge plane).  Positive scale = closes inward,
        //   negative = flares outward.
        // End edges (u=1 or v=1) are negated to match the +u/+v parametric direction.
        u_edge1_deriv_eff = has_sun
            ? (start_u_apex
               ? _apex_fan_tangents(n1_u, points[0][0], points[1])
               : _coplanar_inward_tangents(n1_u, points[0], points[1]))
            : has_fesu ? flat_su_der
            : u_edge1_deriv,
        u_edge2_deriv_eff = has_eun
            ? [for (v = (end_u_apex
               ? _apex_fan_tangents(n2_u, points[n_rows-1][0], points[n_rows-2])
               : _coplanar_inward_tangents(n2_u, points[n_rows-1], points[n_rows-2]))) -v]
            : has_feeu ? flat_eu_der
            : u_edge2_deriv,
        v_edge1_deriv_eff = has_svn
            ? (start_v_apex
               ? _apex_fan_tangents(n1_v, points[0][0],
                                    [for (k = [0:1:n_rows-1]) points[k][1]])
               : _coplanar_inward_tangents(n1_v,
        ...
        v_edge2_deriv_eff = has_evn
            ? [for (v = (end_v_apex
               ? _apex_fan_tangents(n2_v, points[0][n_cols-1],
                                    [for (k = [0:1:n_rows-1]) points[k][n_cols-2]])
               : _coplanar_inward_tangents(n2_v,
