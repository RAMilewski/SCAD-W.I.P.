...
// via penalized normal equations, avoiding the indefinite KKT saddle-point
// system and the Schur complement's sensitivity to near-rank-deficient A.
//
...
//
// Formulation:  min  P^T·R·P + μ·‖A·P − rhs‖²
// Normal equations: (R + μ·A^T·A)·P = μ·A^T·rhs
//
// The system matrix R + μ·A^T·A is M×M symmetric positive-definite
// (R is PSD, μ·A^T·A is PSD, and their null spaces don't overlap for
// any sensible interpolation setup).  BOSL2's QR-based linear_solve()
// handles this reliably.
//
// With μ=1e8, interpolation accuracy is ~1e-6 relative to data scale —
// negligible for all practical purposes.
//
// Returns list of M control points, or undef if the solve fails.

function _kkt_solve(R, A, rhs, mu=1e8) =
    let(
        M      = len(R),
        N_rows = len(A),
        dim    = len(rhs[0]),
        ...
        ATA    = [for (i = [0:1:M-1])
                     [for (j = [0:1:M-1])
                         sum([for (k = [0:1:N_rows-1]) A_T[i][k] * A[k][j]])
                     ]],
        // A^T · rhs  (M × dim)
        ATrhs  = [for (i = [0:1:M-1])
                     [for (d = [0:1:dim-1])
                         sum([for (k = [0:1:N_rows-1]) A_T[i][k] * rhs[k][d]])
                     ]],
        // System matrix: R + μ · A^T · A  (M × M, SPD)
        sys    = [for (i = [0:1:M-1])
                     [for (j = [0:1:M-1])
                         R[i][j] + mu * ATA[i][j]
                     ]],
        // Right-hand side: μ · A^T · rhs  (M × dim)
