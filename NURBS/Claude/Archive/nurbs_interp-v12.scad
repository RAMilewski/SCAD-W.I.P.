...
function _nurbs_interp_closed_basic(points, p, centripetal) =
    let(
        ...
        // Guard: each active span [U[p+k], U[p+k+1]) must contain exactly
        // one collocation param.  For n params over n spans this is equivalent
        // to no span having more than one.  The P&T periodic knot averaging
        // should guarantee this, but fails for highly non-uniform chord-length
        // data (e.g. two adjacent short chords next to a very long one).
        // When violated the collocation matrix is ill-conditioned and
        // linear_solve() returns wildly wrong control points.
        ...
        _chk = assert(max(_span_counts) <= 1,
                      str("nurbs_interp (closed): ill-conditioned collocation — ",
                          "knot span counts are ", _span_counts, ". ",
