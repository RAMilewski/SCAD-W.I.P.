...
function _nurbs_interp_closed(points, degree, method, deriv, curvature,
    ...
    has_corners
      ...
      : (has_dl || has_cl || extra_pts > 0)
        ? let(
              _raw_c = _closed_constrained_solve(points, p, method, deriv, curvature,
                                                  0, extra_pts, smooth),
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="centripetal",
    ...
    // so eff_der[i] is always defined here (no impossible-case guards needed).
        ...
        for (i = [0:1:np-1])
            if (!is_undef(eff_curv[i])) {
                ...
                if (kn < 1e-12) {
                    ...
                    // the exact derivative direction, same green as osculating circles.
                    ...
                    // N_hat: unit vector from data point toward centre of curvature.
                    N_hat = is_num(cv)
                        ? // Signed scalar (2D): rotate unit tangent 90° by sign.
                          let(t_hat = unit(eff_der[i]))
                          (cv >= 0) ? [-t_hat[1],  t_hat[0]]
                                    : [ t_hat[1], -t_hat[0]]
                        : // Dim-projected vector: normalise directly.
                          cv / kn;
                    r   = 1 / kn;
                    ...
                    color([0,1,0,0.1])
                        if (is2d) {
                            ...
                            binom = cross(unit(eff_der[i]), N_hat);
