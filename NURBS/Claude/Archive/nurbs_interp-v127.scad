...
// uniformly-parameterized closed curves), spans are chosen at evenly-
// spaced indices floor(g*n/k_eff) for g=0..k_eff-1.  This prevents
// all extra knots from clustering at the seam of a closed curve — a
// clustering that otherwise occurs because the lexicographic sort
// tiebreaker always selects the highest-indexed equal-width spans.
...
function _widest_span_params(bar_knots, k) =
    ...
    // If all k_eff picks come from equal-width spans, spread them evenly.
    n_eq >= k_eff
    ? [for (g = [0:1:k_eff-1])
           let(i = floor(g * n / k_eff))
