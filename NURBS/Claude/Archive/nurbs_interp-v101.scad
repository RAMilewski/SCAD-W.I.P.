...
// Add a small Tikhonov regularization ε·I to a square matrix.
// Makes a positive-semidefinite R strictly positive-definite,
// which stabilizes the KKT saddle-point system for linear_solve().

function _regularize(R, eps=1e-10) =
    [for (i = [0:1:len(R)-1])
        [for (j = [0:1:len(R)-1])
            R[i][j] + (i == j ? eps : 0)
        ]
...
function _nurbs_interp_clamped_basic(points, p, method, smooth=2) =
    ...
    control != []
    ? [control, knots, 0]
    : // Singular system — fall back to KKT regularization.
      let(
        A_T  = transpose(N_mat),
        R_raw = smooth <= 2
...
function _nurbs_interp_clamped_constrained(points, p, method, eff_der, eff_curv,
    ...
    : let(
        ...
        // [R   A^T] [P]   [  0  ]
        // [A    0 ] [Λ] = [rhs  ]
        // where R = L^T*L (smooth=1,2) or bending-energy matrix (smooth=3),
        // plus ε·I regularization for numerical stability.
...
function _closed_basic_solve(points, n, p, method, rot, smooth=2) =
    ...
    : // Singular — fall back to KKT regularization.
      let(
        M    = n,
        A_T  = transpose(N_mat),
        R_raw = smooth <= 2
