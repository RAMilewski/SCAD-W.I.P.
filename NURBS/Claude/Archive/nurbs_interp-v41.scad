...
// clamped segments at each corner index.  Returns a segmented result
// [undef, undef, 0, segments] where segments is a list of regular
// [control, knots, 0] tuples; callers use _nurbs_is_segmented() to detect.
//
// corner_idxs — sorted list of interior split indices from eff_der.
// NaN entries are replaced with undef for the segment end-points so each
// sub-segment sees clean free endpoints.
...
function _nurbs_interp_clamped_corners(points, p, method, eff_der, eff_curv, corner_idxs) =
    let(
        ...
        segments   = [for (s = [0:n_segs-1])
            ...
            _nurbs_interp_clamped(seg_pts, p, method,
                                  seg_der, undef, undef,
...
function nurbs_interp_curve(points, degree, splinesteps=16,
    ...
    _nurbs_is_segmented(result)
        ? let(
              segs  = result[3],
              paths = [for (seg = segs)
                           nurbs_curve(seg[0], degree, splinesteps=splinesteps,
...
module debug_nurbs_interp(points, degree, splinesteps=16, method="dynamic",
    ...
    if (_nurbs_is_segmented(result)) {
        for (seg = result[3])
            debug_nurbs(seg[0], degree, splinesteps=splinesteps,
