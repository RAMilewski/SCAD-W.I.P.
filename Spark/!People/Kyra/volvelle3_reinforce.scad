...
module bottom() {
    ...
    diff(){
        cyl(d=dmax - 2.5 * wall, h = wall, anchor=BOT) {
            position(TOP) tube(od = dmax - 2.5 * wall/2, h = zmax - wall - .5, 
                irounding2 = wall/2, orounding2 = wall, anchor = BOT);
            position(TOP) tube(id = dsnap+snapgap, h = 2, orounding1 =  -1, anchor = BOT){
                position(TOP) torus(d_maj = dsnap+snapgap, d_min = .75, anchor = TOP);
              /*tag("remove") zrot_copies(n=teeth, r = dsnap-2) position(TOP) cuboid([dsnap/2,toothgap,2], anchor = TOP);  replace with : snap reinforcement below*/
snap_radius = dsnap-2;        // inner edge of teeth
reinforce = 1.2;               // thickness outward
cyl_thickness = 2;             // cylinder wall thickness at this level
snap_depth_total = cyl_thickness + reinforce;
snap_height = 2;             // Z unchanged
toothgap = 0.5;
snap_fillet = 0.2;

tag("remove")
zrot_copies(n=teeth, r = snap_radius + snap_depth_total/6)
    position(TOP)
    cuboid(
        [snap_depth_total, toothgap, snap_height],
        rounding = snap_fillet,
        edges = "Z",
        anchor = TOP
    );
            }
