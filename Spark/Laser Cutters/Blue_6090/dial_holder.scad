...
module holder() {
    diff() {
        tube(id = d1, wall = wall, h = zdim, anchor = BOT){
            position(LEFT) right(1) cuboid([2*wall,2*wall,zdim], rounding=zdim/2, edges="Y", except=RIGHT, anchor=RIGHT);
            position(LEFT) tag("remove") cuboid([4*wall, 2, zdim]);
            position(RIGHT) left(1) cuboid(bridge, anchor = LEFT)
                position(RIGHT) left(1) highlight_this() tube(id = d2, wall = wall, h = zdim, anchor = LEFT){
