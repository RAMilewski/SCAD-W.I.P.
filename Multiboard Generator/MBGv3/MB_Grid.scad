...
module mb_grid () {
    union() {
        grid_copies(n = grid, spacing = mu) octohole();
        move(-mu/2) grid_copies(n = alt_grid, spacing = mu) thread_hole();
        edges();
        corners();
    }
}
...
module make_edge(edge,theta) {
    if (edge == 1) border_wedge(theta); 
            if (edge == 2) move(-mu/2) thread_hole();
            if (edge == 3) {
                if ($idx % 2) border_wedge(theta);
            }
            if (edge == 4) {
                if ($idx % 2) {border_wedge(theta);} 
                else {move(-mu/2) thread_hole();}
            } 
}
...
module make_corner(type,theta,phi) {
     if(type == 1) move(-mu/2) thread_hole(); 
