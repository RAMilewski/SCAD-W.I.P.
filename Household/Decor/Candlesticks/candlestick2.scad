...
part = "ballast";  // ["ballast", "bottom", "top"]

if (part == "ballast") ballast_plate();
if (part == "bottom") back_half(s = 200) bottom();
if (part == "top") top();

left(50) xrot(90) ruler();
                    ...
                    tag("remove") position(BOT) cyl(d1 = cyldim2[0], d2 = cyldim2[1], h = cyldim2.z, anchor=BOT)
                        tag("remove") position(TOP) rotate_sweep(path,360)   
                            tag("remove") position(TOP) threaded_rod(d=INCH/4, height=20, pitch=INCH/20, 
                                lead_in_shape = "smooth", bevel1 = -1, bevel2 = true, $slop = .15, $fa=1, $fs=1, anchor = BOT);  
                }
...
module ballast_plate() {
    diff() {
        cyl(d1 = cyldim2[0], d2 = cyldim2[1], h = cyldim2.z, anchor=BOT) {
            tag("remove") position(BOT) cuboid([30,5,cyldim2.z/2], rounding = 2.5, edges = TOP, anchor = BOT);
            tag("keep") position(TOP) threaded_rod(d=INCH/4, height=47, pitch=INCH/20, 
