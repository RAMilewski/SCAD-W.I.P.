    ...
    attachable(anchor, h = h, d1 = cyldim[0], r2 = core) {
        ...
        diff() {
            cyl(d1 = cyldim[0], d2 = cyldim[1], h = cyldim.z, anchor=BOT) {
                ...
                down(cyldim.z)  {
                    tag("remove") position(BOT) down(0.005) zscale(0.02) cyl(d1 = cyldim2[0], d2 = cyldim2[1], h = cyldim2.z, anchor=BOT)
...
module ballast_plate() {
    diff() {
        cyl(d1 = cyldim2[0], d2 = cyldim2[1], h = cyldim2.z/2, anchor=BOT) {
            ...
            tag("keep") position(TOP) threaded_rod(d=7.5, height=60, pitch=1.5, 
